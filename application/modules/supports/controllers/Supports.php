...
class Supports extends MX_Controller {
    ...
    function active()
    {
		...
		$this->template
		->set_layout('users')
		->build('logistics',isset($data) ? $data : NULL);
    }

    function logistics_page()
    {
		$this->load->module('layouts');
        $this->load->library('template');
        $this->template->title(lang('logistics').' - '.$this->company_name);
        ...
        $this->template
                ->set_layout('users')
                ->build('logistics/all_assets', isset($data) ? $data : null);    
	}
    ...
    function e_notices()
    {
		$this->load->module('layouts');
        $this->load->library('template');
        $this->template->title(lang('support').' - '.$this->company_name);
        $data['page'] = lang('support');
        $data['form'] = true;
        $data['datatables'] = true;
        $data['all_assets'] = $this->db->get('user_assets')->result_array();
        $this->template
                ->set_layout('users')
                ->build('supports/e_notices', isset($data) ? $data : null);    
	}
    ...
    function procurement()
    {
        ...
        $this->template
                ->set_layout('users')
                ->build('logistics/procurement', isset($data) ? $data : null);    
	}
    ...
    function suppliers()
    {
        ...
        $this->template
                ->set_layout('users')
                ->build('logistics/suppliers', isset($data) ? $data : null);    
	}
