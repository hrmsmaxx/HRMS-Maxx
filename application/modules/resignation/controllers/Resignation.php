...
class Resignation extends MX_Controller {
    ...
    function index()
    {
		if($this->tank_auth->is_logged_in()) { 
                ...
                $this->template->title(lang('resignation').' - '.$company_name); 
 				$data['datepicker'] = TRUE;
				$data['form']       = TRUE; 
                $data['page']       = lang('all_policies');
                $data['sub_page']       = lang('resignation');
        ...
        $a=1;
         foreach ($list as $resignation) {
           ...
           $no++;
            $row = array();
            $row[] = $a++;
            $row[] = '<div class="user_det_list">
                        <a href="'.base_url().'employees/profile_view/'.$resignation->user_id.'"> <img class="avatar"  src="'.base_url().'assets/avatar/'.$resignation->avatar.'"></a>
                        <h2><span class="username-info">'.$resignation->fullname.'</span>
                        <span class="userrole-info"> '.$resignation->designation.'</span>
                        <span class="username-info">'. $resignation->id_code.'</span></a></h2>
                    </div>';
