...
class Time_sheets extends MX_Controller {
...

	{
		...
		$this->template->title(lang('timesheets').'-'.$company_name);
		...
		$department = explode(',', $data['dept_id']);
		...
		if($user_type == 'supervisor') {
			$this->db->where_in(array('branch_id', $branch)); 
			$this->db->where_in(array('department_id', $department)); 
		}
		...
		$data['employee'] = $this->db->select('*')
					...
					->get()
					->result();
		echo 'emp<pre>'; print_r($data['employee']); exit;*/		
		if(!$_POST){
			//echo 'if'; //exit;
				...
				if(($this->tank_auth->user_role($this->tank_auth->get_user_type()) != 'admin'))
	...
	public function manual_timesheets()

	{
		...
		$this->template
			 ...
			 ->build('manual_timesheets',isset($data) ? $data : NULL);

	}
	public function time_sheet_excel(){
		// if(!empty($_POST)){

		$html = $this->load->view('excel/timesheet_excel',$data,true);
