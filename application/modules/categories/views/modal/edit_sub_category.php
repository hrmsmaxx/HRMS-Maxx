<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<h4 class="modal-title"><?=lang('edit_sub_category')?></h4>
		...
		<div class="modal-body">
			<?php $attributes = array('class' => 'bs-example','id'=>'settingsDesignationForm'); echo form_open_multipart('categories/edit_sub_category/'.$sub_details['sub_id'], $attributes); ?>
				<div class="form-group">
					<label><?=lang('categories')?> <span class="text-danger">*</span></label>
					...
					<select name="cat_id" id="cat_id" required="required" class="form-control" >
						<option value="" selected disabled><?=lang('categories')?></option>
						<?php
						$categories = $this->db->get_where('budget_category', array('subdomain_id'=>$this->session->userdata('subdomain_id')))->result();
						if (!empty($categories)) { 
						...
						<?php } }else{ ?>
						<option value=""><?php echo lang('no_records_found')?></option>
						<?php } ?>
					</select>
				</div>
				<div class="form-group">
				...
				<div class="submit-section"> 
					<button id="settings_designation_submit" class="btn btn-primary submit-btn"><?=lang('save')?></button>
				</div>
			</form>
		</div>
	</div>
