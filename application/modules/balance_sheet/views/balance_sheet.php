<?php if(!empty($this->uri->segment(3))){
  ...
  $active_year = date("Y");
      ...
      <div class="content container-fluid">
		<div class="row">
			<div class="col-sm-12">
				<h4 class="page-title"><?=lang('balance_sheet')?></h4>
			</div>
		</div>
        ...
        <div class="tab-content">           
           ...
           <?php  for ($i=$start_years; $i <=$current_year ; $i++) { ?>
            <div id="statement_<?php echo $i;?>" class="tab-pane <?php echo ($i == $active_year )?'active':'';?>">
              <div class="card-box">
                <h4 class="card-title"><?=lang('balance_sheet')?> <?php echo $i;?></h4>
                ...
                <table class="table table-hover accounts-table budget-table m-b-0 table-nowrap">
                  ...
                  <tbody id="asset_cont_<?php echo $i;?>">
                    <tr class="title-row">
                      <td colspan="10"><strong><?=lang('all_assets')?></strong></td>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-xs add-new" id="asset_add" data-year ="<?php echo $i;?>" onclick="asset_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> <?=lang('add_assets')?></button></td>
                    </tr>
                    <?php $this->db->select('*');
                                ...
                                <div class="modal-dialog">
                                  <div class="modal-content modal-md">
                                    <div class="modal-header">
                                      <h4 class="modal-title"><?=lang('delete_balance_sheet')?>( <?php echo ucfirst($asset['title']);?> )</h4>
                                    </div>
                  ...
                  <tbody>
                    <tr class="total-row">
                      <td><strong><?=lang('total_assets')?></strong></td>
    ...
    color: red;"'?>><?php echo $total_assets; ?></strong></td>
                  ...
                  <tbody id="liabilitie_cont<?php echo $i;?>">
                    <tr class="title-row">
                      <td colspan="10"><strong><?=lang('liabilities')?></strong></td>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-xs add-new" id="liabilitie_add" data-year ="<?php echo $i;?>" onclick="liabilitie_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> <?=lang('add_liabilities')?></button></td>
                    </tr>
                    <?php $this->db->select('*');
                          $this->db->where("year",$i);
                            ...
                            foreach ($liabilities as $liabilitie) {?>
                              <tr>
                                ...
                                <div class="modal-dialog">
                                  <div class="modal-content modal-md">
                                    <div class="modal-header">
                                      <h4 class="modal-title"><?=lang('delete_liabilities')?>( <?php echo $liabilitie['title']?> )</h4>
                                    </div>
                  ...
                  <tbody>
                    <tr class="total-row">
                      <td><strong><?=lang('total_liabilities')?></strong></td>
                      <?php $this->db->select('sum(jan) as jan,sum(feb) as feb,sum(mar) as mar,sum(apr) as apr,sum(may) as may,sum(jun) as jun,sum(jul) as jul,sum(aug) as aug,sum(sep) as sep,sum(oct) as oct,sum(nov) as nov,sum(dece) as dece');
                          $this->db->where("year",$i);
                          $this->db->where("type","liabilitie");
                  ...
                  <tbody id="shareholder_cont<?php echo $i;?>">
                    <tr class="title-row">
                      <td colspan="10"><strong><?=lang('shareholders_equity')?></strong></td>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-xs add-new" id="shareholder_add" data-year ="<?php echo $i;?>" onclick="shareholder_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> <?=lang('Add')?> <?=lang('shareholders_equity')?></button></td>
                    </tr>
                    <?php $this->db->select('*');
                          $this->db->where("year",$i);
                            ...
                            foreach ($shareholders as $shareholder) {?>
                              <tr>
                                ...
                                <div class="modal-dialog">
                                  <div class="modal-content modal-md">
                                    <div class="modal-header">
                                      <h4 class="modal-title"><?=lang('delete_balance_sheets')?> <?=lang('shareholders_equity')?>( <?php echo $shareholder['title']?> )</h4>
                                    </div>
                  ...
                  <tbody>
                    <tr class="total-row">
                      <td><strong><?=lang('shareholders_equity')?></strong></td>
                      <?php $this->db->select('sum(jan) as jan,sum(feb) as feb,sum(mar) as mar,sum(apr) as apr,sum(may) as may,sum(jun) as jun,sum(jul) as jul,sum(aug) as aug,sum(sep) as sep,sum(oct) as oct,sum(nov) as nov,sum(dece) as dece');
                          $this->db->where("year",$i);
                          $this->db->where("type","shareholder");
                  ...
                  <tbody>
                    <tr>
                      <td><strong><?=lang('total_liabilities')?> & <?=lang('shareholders_equity')?></strong></td>
                      <?php 
