...
class Calendar extends MX_Controller {
    ...
    function index()
    {
                ...
                $this->template->title(lang('calendar').'-'.config_item('company_name'));
                $data['fullcalendar'] = TRUE;
                $data['datepicker'] = TRUE;
                $data['form'] = TRUE;
                $data['page'] = lang('calendar');
                $data['sub_page'] = lang('calendar');
                $this->template
                ->set_layout('users')
        ...
        function event($type, $id) {
            ...
            switch ($type) {
                ...
                case 'events':
                    ...
                    $data['incidents'] = $incidents[0];
                    // print_r($data['incidents']); exit;
                    break;
            }
         ...
         function get_events()
         {
        $data = $this->db->select("*")->from('dgt_events')->where('subdomain_id',$this->session->userdata('subdomain_id'))->get()->result_array(); 
        $return_array = [];
