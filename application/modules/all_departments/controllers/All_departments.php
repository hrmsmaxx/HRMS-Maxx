...

{
...

    {
        ...
        $data['sub_page'] = lang('all_departments');
    ...
    function departments(){
        ...
        if ($_POST) {
            ...
            $department = $this->db->get_where('dgt_departments',array('deptname'=> $_POST['deptname'],'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
            ...
            if(empty($department)){   
                ...
                $this->session->set_flashdata('tokbox_error', lang('department_name_already_exist'));
            }
            ...
            $this->load->view('modal/add_department', $data);
        ...
        if ($_POST) {
            ...
            unset($_POST['settings']);
            $_POST['subdomain_id'] = $this->session->userdata('subdomain_id');
            $designation = $this->db->get_where('dgt_designation',array('designation'=>$_POST['designation'],'department_id'=>$_POST['department_id'],'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
            $desi_count = $this->db->order_by('id_code','DESC')->get_where('dgt_designation',array('subdomain_id'=>$this->session->userdata('subdomain_id'),'department_id'=>$_POST['department_id']))->row()->id_code;                 
            $id_code = $desi_count+1;
            // echo $desi_count; exit;
            // print_r($designation);die;
            // echo $this->db->last_query(); exit;
            $_POST['id_code'] = $id_code;
            if(empty($designation)){
                App::save_data('designation',$this->input->post());
                $this->session->set_flashdata('tokbox_success', lang('position_added_successfully'));
            }else{
                $this->session->set_flashdata('tokbox_error', lang('position_name_already_exist'));
            }
...

    {
        ...
        if($_POST){
            ...
            $this->session->set_flashdata('tokbox_success', lang('designation_updated'));
...

    {
        ...
        if($_POST){
            ...
            $this->session->set_flashdata('tokbox_success', lang('designation_deleted_successfully'));
