...
class Tickets extends MX_Controller {
...

	{
        ...
        $this->timezone = $systems['timezone']?$systems['timezone']:config_item('timezone');
        $this->default_currency = $systems['default_currency']?$systems['default_currency']:config_item('default_currency');
        $this->file_max_size = $systems['file_max_size']?$systems['file_max_size']:config_item('file_max_size');
    	$this->allowed_files = $systems['allowed_files']?$systems['allowed_files']:config_item('allowed_files');

		$this->load->module('layouts');

		$this->load->library(array('template','form_validation','encrypt'));

		$this->template->title(lang('tickets').' - '.$this->company_name);
...

	{
		...
		$data['page'] = lang('all_apps');

		$data['sub_page'] = lang('tickets');
...

	{
		...
		if ($this->input->post()) {
...

			{
				...
				$time_estimation = $this->db->where('priority',$_POST['priority'])->get('priorities')->row();
				...
				if (!User::is_admin()) {
					...
					$insert['subdomain_id'] = $this->session->userdata('subdomain_id'); 
	...
	function _notify_ticket_reopened($ticket){
			...
			// }else{
				...
				$params1['message'] = str_replace("{RECIPIENT}",$email,$message);
...
$staff_members = $this->db->where(array('department_id'=>$info->department,'activated'=>1,'banned'=>0)) -> get('users')->result();
	...
	function ticket_time_expired_email_notification(){
		...
		$tickets = $this->db->where('status !=','closed')->get('tickets')->result();
	...
	function _upload_attachment($data){
		...
		$config['allowed_types'] = $this->allowed_files;
	...
	function _admin_tickets($archive = FALSE,$filter_by = NULL){
		...
		if($filter_by == NULL) return Ticket::by_where(array('archived_t !=' => '1','subdomain_id'=>$this->session->userdata('subdomain_id')));



		if ($archive) return Ticket::by_where(array('archived_t' => '1','subdomain_id'=>$this->session->userdata('subdomain_id')));
		...
		switch ($filter_by) {
			...
			return Ticket::by_where(array('archived_t !='=>'1','status' => 'open','subdomain_id'=>$this->session->userdata('subdomain_id')));

			break;

			case 'closed':

			return Ticket::by_where(array('archived_t !='=>'1','status' => 'closed','subdomain_id'=>$this->session->userdata('subdomain_id')));

			break;

			case 'pending':

			return Ticket::by_where(array('archived_t !='=>'1','status' => 'pending','subdomain_id'=>$this->session->userdata('subdomain_id')));

			break;

			case 'resolved':

			return Ticket::by_where(array('archived_t !='=>'1','status' => 'resolved','subdomain_id'=>$this->session->userdata('subdomain_id')));
			...
			return Ticket::by_where(array('archived_t !='=>'1','subdomain_id'=>$this->session->userdata('subdomain_id')));
