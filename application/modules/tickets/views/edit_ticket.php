           ...
           ?>
			 ...
			 <input type="hidden" name="id" value="<?=$info->id?>">
				...
				<div class="col-lg-6">
					...
					<?php 
					$priorities = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->order_by('hour','DESC')->get('priorities')->result();
						foreach ($priorities as $p): ?>
					<option value="<?=$p->priority?>" <?php echo($info->priority == $p->priority)?"selected":"";?>><?=lang(strtolower($p->priority))?></option>
					<?php endforeach; ?>
					</select> 
					</div> 
				</div>
...
<?php if (User::is_admin()) { ?>
		...
		<?php } ?>
			...
			<div class="form-group">
				...
				<div class="col-lg-9">
				<textarea name="body" class="form-control foeditor-ticket-messageU"><?=$info->body?></textarea>
				<div class="row">
				<div class="col-md-6">
				<label id="editTicket_message_error" class="error display-none" style="position:inherit;top:0;font-size:15px;"><?php echo lang('message_must_not_empty')?></label>
				</div>
				</div>
				</div>
				</div>
