...
class Asset_categories extends MX_Controller

{
    ...
    function categoiress(){
        ...
        if ($_POST) {
            ...
            $this->db->where('category_name',$_POST['category_name']);
             if($this->db->get('asset_category')->num_rows() == 0)
            {
                $this->db->insert('asset_category', $_POST);
            ...
            // App::save_data('asset_category',$this->input->post());
        ...
        if ($_POST) {
            ...
            App::save_data('asset_subcategory',$this->input->post());
...

    {
        ...
        if($_POST){
            ...
            $this->db->where('sub_id',$sub_id) -> update('asset_subcategory',$this->input->post());
            ...
            $data['sub_details'] = $this->db->get_where('asset_subcategory',array('sub_id'=>$sub_id))->row_array();
...

    {
        ...
        if($_POST){
            ...
            $this->db->where('sub_id',$sub_id) -> delete('asset_subcategory');
            ...
            $data['des_details'] = $this->db->get_where('asset_subcategory',array('sub_id'=>$sub_id))->row_array();
    ...
    function edit_categories($cat_id = NULL){
        ...
        if ($_POST) {
            ...
            if(isset($_POST['delete_category']) AND $_POST['delete_category'] == 'on'){
                ...
                $this->db->where('cat_id',$_POST['cat_id']) -> delete('asset_category');
                ...
                $this->db->where('cat_id',$_POST['cat_id']) -> update('asset_category',$this->input->post());
            ...
            $data['category_info'] = $this->db ->where(array('cat_id'=>$cat_id)) -> get('asset_category') -> result();
    ...
    function view_sub_categories($cat_id){
        ...
        $data['all_subcategories'] = $this->db->get_where('asset_subcategory',array('cat_id'=>$cat_id))->result();
