...
class Leave_settings extends MX_Controller {
    ...
    function index()
    {
		if($this->tank_auth->is_logged_in()) { 
                ...
                $this->template->title(lang('leaves')); 
     ...
     function subadmin_leave(){
        ...
        for($i=0; $i<count($result); $i++) {
            ...
            $result[$i]['subdomain_id'] = $this->session->userdata('subdomain_id');
        }
     ...
     function leave_types(){
        if ($this->input->post()) {            
            ...
            $this->session->set_flashdata('tokbox_success', lang('leave_type_updated_successfully'));
            } 
    ...
    function update_annual_leaves()
    {
        ...
        $anu_leaves = $this->db->get_where('common_leave_types',array('leave_keys'=>'annual_leaves','subdomain_id'=>$this->session->userdata('subdomain_id'),'status'=>'0'))->row_array();
        $cr_leaves = $this->db->get_where('common_leave_types',array('leave_keys'=>'carry_forward','subdomain_id'=>$this->session->userdata('subdomain_id'),'status'=>'0'))->row_array();
    ...
    function update_hospitalisation_leave()
    {
        ...
        $this->db->where('leave_keys','hospitalisation');
        $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'));
        $this->db->update('common_leave_types',$res);
    ...
    function update_extra_leave(){
        $extra_leaves = $this->input->post('extra_leaves');
        $res =array(
            'leave_days' => $extra_leaves
            );
        $this->db->where('leave_keys','custom_leave_type');
        $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'));
        $this->db->update('common_leave_types',$res);
        echo $extra_leaves; exit;
    }
    ...
    function update_carry_forward_leave()
    {
        ...
        $this->db->where('leave_keys','carry_forward');
        $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'));
        $this->db->update('common_leave_types',$res);
        $check_leaves = $this->db->get_where('yearly_leaves',array('years'=>$cr_yr))->row_array();
        $anu_leaves = $this->db->get_where('common_leave_types',array('leave_keys'=>'annual_leaves','subdomain_id'=>$this->session->userdata('subdomain_id'),'status'=>'0'))->row_array();
    ...
    function update_earned_leave()
    {
        ...
        $this->db->where('leave_keys','earned_leaves');
        $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'));
        $this->db->update('common_leave_types',$res);
    ...
    function add_new_leave_type()
    {
        ...
        $res = array(
            ...
            'leave_keys' => 'custom_leave_type',
            'subdomain_id' => $this->session->userdata('subdomain_id')
        );
        $this->db->insert('common_leave_types',$res);
        $this->session->set_flashdata('tokbox_success', lang('new_leave_type_added_successfully'));
        redirect('leave_settings');
    }
    ...
    function policy_delete($id)
    {
        ...
        {
            $this->session->set_flashdata('tokbox_success', lang('policy_deleted_successfully'));
            echo "1";
        }
        else
        {
            $this->session->set_flashdata('tokbox_danger', lang('policy_deleted_failed'));
            echo "0";
        }
        exit;
