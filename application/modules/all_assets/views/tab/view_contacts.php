    ...
    <div class="col-md-12">
        <div class="panel panel-table">
            <div class="panel-heading">
				<div class="row">
					<div class="col-xs-6">
						<h3 class="panel-title"><?php echo lang('contacts')?></h3>
					</div>
					<div class="col-xs-6">
						<a href="<?=base_url()?>contacts/add/<?=$l->co_id?>" class="btn btn-xs btn-success pull-right" data-toggle="ajaxModal"><i class="fa fa-plus"></i> <?php echo lang('add_contact')?></a>
					</div>
				</div>
			</div>
					...
					<table id="table-client-details-1" class="table table-striped custom-table m-b-0 AppendDataTables">
						...
						<?php foreach (Lead::contacts($l->co_id) as $key => $contact) { ?>
							<tr>
								...
								<td><?=$login_time?> </td>
								<td class="text-right">
									<a href="<?=base_url()?>leads/make_primary/<?=$contact->user_id?>/<?=$l->co_id?>" class="btn btn-default btn-xs" title="<?php echo lang('primary_contact')?>" >
										<i class="fa fa-chain <?php if ($l->primary_contact == $contact->user_id) { echo "text-danger"; } ?>"></i> </a>
									<a href="<?=base_url()?>contacts/update/<?=$contact->user_id?>" class="btn btn-success btn-xs" title="<?php echo lang('edit')?>"  data-toggle="ajaxModal">
										<i class="fa fa-edit"></i> </a>
									<a href="<?=base_url()?>users/account/delete/<?=$contact->user_id?>" class="btn btn-danger btn-xs" title="<?php echo lang('delete')?>" data-toggle="ajaxModal">
