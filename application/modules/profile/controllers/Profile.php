...
class Profile extends MX_Controller {
	...
	function settings()
	{
		...
		if ($this->form_validation->run() == FALSE) // validation hasn't been passed
			...
			$this->template->title(lang('profile').' - '.$this->company_name);
			$data['page'] = lang('home');
			$data['form'] = TRUE;
			$this->template
			->set_layout('users')
	...
	{		
		...
		if(file_exists($_FILES['userfile']['tmp_name']) || is_uploaded_file($_FILES['userfile']['tmp_name'])) {
			$current_avatar = User::profile_info(User::get_id())->avatar;
				...
				// echo lang('avatar_uploaded_successfully');die;
	...
	function activities()
	{
		//echo 'post<pre>'; print($this->input->post()); exit;
	$this->load->module('layouts');
	$this->load->library('template');
	$this->template->title(lang('profile').' - '.$this->company_name);
		...
		if($this->tank_auth->user_role($this->tank_auth->get_role_id()) != 'admin'){
			...
			$activity = $this->db->select('*')
			  ...
			  ->get()->result();
					...
					$activity =  $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->order_by('activity_date','DESC')->get('activities')->result();
			}
		}
