...
class Projects extends MX_Controller {
...

    {
        ...
        $data = array(
            ...
            'sub_page' => lang('all_projects'),

            'projects' => $this->_project_list($archive, $postdata),
...

    {
        ...
        if ($this->input->post()) {
...

            {
                ...
                //echo 'post<pre>'; print_r($this->input->post()); exit;
                $project_id = Project::save($this->input->post());
                
                /*$this->db->insert('1projects', $this->input->post()); exit;
                if($this->db->affected_rows() > 0) {
                    ...
                    $this->session->set_flashdata('tokbox_error', lang('project cannot added'));
                     redirect('projects/add');
                }*/
            ...
            /*if($this->session->userdata('role_name')=='admin' && $this->session->userdata('branch_id') != '') {
                ...
                $subdomain_id = $this->session->userdata('subdomain_id');
                $data['branches'] = $this->db->query("SELECT * FROM `dgt_branches` WHERE find_in_set(`id`, '$branch_id') AND `subdomain_id`='".$subdomain_id."' ")->result();
            } else {
                $data['branches'] = $this->db->get_where('dgt_branches', array('subdomain_id'=>$this->session->userdata('subdomain_id')))->result();
            }*/
            ...
            $user_type = $this->session->userdata('role_name'); 

            if($user_type == 'admin') {
                if($this->session->userdata('branch_id') != '') {
                    $this->db->where("branch_id IN (".$this->session->userdata('branch_id').")",NULL, false);
                }
                $this->db->or_where(array('teamlead_id'=>$this->session->userdata('user_id'), 'id!='=>$this->session->userdata('user_id')));
            }
...

    {
        ...
        if ($this->input->post()) {
            ...
            //$data['branches'] = $this->db->get_where('dgt_branches', array('status'=>'1','subdomain_id'=>$this->session->userdata('subdomain_id')))->result();
            /*if($this->session->userdata('role_name')=='admin' && $this->session->userdata('branch_id') != '') {
                $branch_id = $this->session->userdata('branch_id');
                $subdomain_id = $this->session->userdata('subdomain_id');
                $data['branches'] = $this->db->query("SELECT * FROM `dgt_branches` WHERE find_in_set(`id`, '$branch_id') AND `subdomain_id`='".$subdomain_id."' ")->result();
    ...
    function _project_list($archive = NULL, $postdata = NULL){


        if (User::is_admin() || User::perm_allowed(User::get_id(),'view_all_projects'))
        //if (($this->session->userdata('role_name') == 'admin' && $this->session->userdata('branch_id') != '') || User::perm_allowed(User::get_id(),'view_all_projects'))

            return $this->_admin_projects($archive,$this->filter_by,$postdata);
    ...
    function _admin_projects($archive = FALSE,$filter_by = NULL, $postdata=NULL){

        $branch_id = $this->session->userdata('branch_id');
        if($postdata != NULL) {
            if($postdata['project_title'] != NULL) {
                $this->db->like('p.project_title', $postdata['project_title']);
            }
            if($postdata['client_name'] != NULL) {
                $this->db->or_like('c.company_name', $postdata['client_name']);
            }
            $projects = $this->db->select('p.*')
                ->from('projects p')
                ->join('assign_projects ap', 'ap.project_assigned=p.project_id', 'left')
                ->join('companies c', 'c.co_id=p.client', 'left')
                ->where(array('p.archived !='=>'1','p.subdomain_id'=>$this->session->userdata('subdomain_id')))
        ...
        if ($archive) {
                //$projects = $this->db->query("SELECT * FROM `dgt_branches` WHERE find_in_set(`id`, '$branch_id') AND `subdomain_id`='".$subdomain_id."' ")->result();
             //$this->db->where("FIND_IN_SET(".$key.", job_title)");
            return Project::by_where(array('archived'=>'1','subdomain_id'=>$this->session->userdata('subdomain_id')));
        }
        if($filter_by != NULL){

            return Project::by_where(array('archived !='=>'1','status' => $filter_by,'subdomain_id'=>$this->session->userdata('subdomain_id')));

        }else{
            return Project::by_where(array('archived !='=>'1','subdomain_id'=>$this->session->userdata('subdomain_id')));
            //return Project::by_where(array("find_in_set('branch_id',$branch_id)",'archived !='=>'1','subdomain_id'=>$this->session->userdata('subdomain_id')));
    ...
    function _staff_projects($archive = FALSE, $filter_by = NULL){
        ...
        if ($archive) {
            ...
            $pr['projects'] =  Project::by_where(array('archived'=>'1','assigned_user'=>User::get_id(),'subdomain_id'=>$this->session->userdata('subdomain_id')),'assign_projects');

            $lead_check = $this->db->get_where('projects',array('assign_lead'=>$this->session->userdata('user_id'),'archived '=>'1','subdomain_id'=>$this->session->userdata('subdomain_id')))->result();
    ...
    public function getBranch() 
    {
        if($this->input->post()){
            ...
            $this->db->select('co_id,company_name');
    ...
    public function getLead() 
    {
        if($this->input->post()){
            ...
            $this->db->from('users');
            $this->db->where(array('branch_id'=>$branch_id, 'is_teamlead'=>'yes')); 
     ...
     public function getEmployee() 
    {
        if($this->input->post()){
            ...
            $this->db->select('id,username');
            $this->db->from('users');
            $this->db->where(array('teamlead_id'=>$lead_id, 'is_teamlead'=>'yes', 'subdomain_id'=>$this->session->userdata('subdomain_id'))); 
            $this->db->order_by("username", "asc");
            $records = $this->db->get()->result_array();
            echo json_encode($records);
            die();
