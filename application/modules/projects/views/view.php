                ...
                <div class="content container-fluid">
					...
					<div class="row">
						...
						<div class="col-sm-6 text-right m-b-30">
							...
							<a href="<?php echo base_url(); ?>projects/edit/<?php echo $p->project_id; ?>" class="btn add-btn"><i class="fa fa-plus"></i> <?=lang('edit_project');?></a>						
						</div>
					</div>
					<!-- /Page Title -->
					...
					<div class="row">
						<div class="col-lg-8 col-xl-9">
							<div class="card">
								<div class="card-body">
									<div class="project-title">
										...
										?>
										<small class="block text-ellipsis m-b-15"><span class="text-xs"><?php echo count($open_tasks); ?></span> <span class="text-muted"><?=lang('open_tasks');?>, </span><span class="text-xs"><?php echo count($completed_tasks); ?></span> <span class="text-muted"><?=lang('tasks_completed');?></span></small>
									</div>
									<?=$p->description; ?>
								</div>
							</div>
							<div class="card">
								<div class="card-body">
				                    <h5 class="card-title m-b-20"><?=lang('upload_image_files');?></h5>
									<div class="row">
										<?php
					$project_files =$this->db->query("SELECT tf.* FROM dgt_tasks AS t LEFT JOIN  dgt_task_files AS tf ON tf.task=t.t_id  WHERE t.project='".$p->project_id."' ORDER BY tf.date_posted ASC")->result_array();
							...
							<div class="card">
								...
								<div class="card-body">
									...
									<h5 class="card-title m-b-20"><?=lang('uploaded_files');?></h5>
												...
												<ul class="files-action">
													...
													<li class="dropdown dropdown-action">
														...
														<div class="dropdown-menu dropdown-menu-right">
															...
															<a class="dropdown-item" href="<?php echo base_url().'projects/downloads/'.$projects_file['path'].'/'.$projects_file['file_name'];?>"><?=lang('download');?></a>
							...
							<div class="project-task">
								...
								<ul class="nav nav-tabs nav-tabs-top nav-justified m-b-0">
									<li class="active"><a href="#all_tasks" data-toggle="tab" aria-expanded="true"><?=lang('all_tasks');?></a></li>
									<li><a href="#pending_tasks" data-toggle="tab" aria-expanded="false"><?=lang('pending_tasks');?></a></li>
									<li><a href="#completed_tasks" data-toggle="tab" aria-expanded="false"><?=lang('completed_tasks');?></a></li>
														...
														<li class="<?php echo $task_progrs; ?> task">
															...
															<div class="task-container">
																...
																<span class="task-action-btn task-check">
																	...
																	<span class="action-circle large complete-btn task_completes" data-id="<?php echo $task['t_id']; ?>" title="<?php if($task_progrs != 'completed') { echo lang('mark_complete'); } else { echo lang('mark_incomplete'); } ?>">
													...
													<ul id="task-list">
													    ...
													    <?php foreach($open_tasks as $taskss){ 
													        ...
													        $assign_member = $this->db->select('*')
																	         ...
																	         $assignrds_name1=$assign_member['fullname'];
														...
														<li class="<?php echo $task_progrss; ?> task">
															...
															<div class="task-container">
																<span class="action-circle large complete-btn task_completes" data-id="<?php echo $taskss['t_id']; ?>" title="<?=lang('mark_complete');?>">
																...
																<span class="task-action-btn task-btn-right">
																	...
																	<span id="assign_tasks<?php echo $taskss['t_id']; ?>">
																				<?php
																				if(!empty($assignrds_name1))
																				{
																					echo'<span class="action-circle large" title="'.$assignrds_name1.'">
																							<img src="'.$pro_pic_teams.'" alt="">
																						</span>';
																				}
																				else
													...
													<ul id="task-list">
													    ...
													    <?php foreach($completed_tasks as $tasss){ 
													        ...
													        $assign_member = $this->db->select('*')
																	         ...
																	         $assignrds_name2=$assign_member['fullname'];
														...
														<li class="<?php echo $task_progr; ?> task">
															...
															<div class="task-container">
																...
																<span class="action-circle large complete-btn task_completes" data-id="<?php echo $tasss['t_id']; ?>" title="<?=lang('mark_incomplete');?>">
																...
																<span class="task-action-btn task-btn-right">
																	...
																	<span id="assign_tasks<?php echo $taskss['t_id']; ?>">
																				<?php
																				if(!empty($assignrds_name2))
																				{
																					echo'<span class="action-circle large" title="'.$assignrds_name2.'">
																							<img src="'.$pro_pic_teams.'" alt="">
																						</span>';
																				}
																				else
						...
						<div class="col-lg-4 col-xl-3">
							...
							<div class="card">
								...
								<div class="card-body">
									...
									<h6 class="card-title m-b-15"><?=lang('project_details');?></h6>
											...
											<tr>
												...
												<?php 
												    ...
												    if($p->progress == 100){
												        ...
												        $pro_status = lang('done');

												    }

												    if($p->progress == 0){

												        $pro_status = lang('not_yet_started');

												    }

												    if($p->progress > 1 ){

												        $pro_status = lang('working');
									...
									<?php 
										$completed_task_count = $this->db->get_where('tasks',array('project'=>$p->project_id,'task_progress'=>'100'))->result_array();
										$open_task_count = $this->db->get_where('tasks',array('project'=>$p->project_id,'task_progress !='=>'100'))->result_array();

										$tot_tasks = count($open_task_count)+count($completed_task_count);
										$task_prog = (count($completed_task_count)/$tot_tasks)*100;
									?>

									<p class="m-b-5"><?=lang('progress');?> <span class="text-success float-right"><?php echo round($task_prog).'%'; ?></span></p>
											...
											<a href="<?php echo base_url(); ?>employees/profile_view/<?php echo $p->assign_lead; ?>">
												...
												<div class="list-item">
													...
													<div class="list-body">
														...
														<span class="message-content"><?=lang('team_leader');?></span>
							...
							<div class="card project-user">
								...
								<div class="card-body">
									...
									<h6 class="card-title"><?=lang('assigned_users');?> <a class="pull-right btn btn-primary btn-xs" data-toggle="ajaxModal" href="<?=base_url()?>projects/team/<?=$p->project_id?>"><i class="fa fa-plus"></i> <?=lang('Add');?></a></h6>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<h3 class="modal-title"><?=lang('assign_leader_to_this_project');?></h3>
