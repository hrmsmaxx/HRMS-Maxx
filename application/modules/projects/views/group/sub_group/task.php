	...
	<div class="row">
		<div class="col-lg-4">
			...
			<div class="card-box m-t-3">
				<h6 class="panel-title m-b-15">Assigned to</h6>
				<ul class="team-members">
					...
					<div class="col-lg-8">
                        ...
                        <div class="panel panel-default m-t-3">
                            <?php $attributes = 'class="m-b-0"'; $attributesID = 'id="taskViewComment"'; echo form_open(base_url().'projects/tasks/comment',$attributes,$attributesID); ?>
                                <div class="panel-body">
                                    ...
                                    <textarea class="form-control foeditor-taskview-comment" name="message" placeholder="<?=$t->task_name?> <?=lang('comment')?>"></textarea>
                                    <div class="row">
                                    <div class="col-md-6">
                                    <label id="taskview_comment_error" class="error display-none" style="position:inherit;top:0" for="commentTextarea">Comment must not empty</label>
                                    </div>
                                    </div>
                                </div>
                                <div class="panel-footer bg-white">
									<button class="btn btn-success pull-right" id="task_view_comment"><?=lang('comment')?></button>
					...
					<table class="table table-striped table-border">
						<tbody>
							<tr>
								...
								<?php 
								$all_task_details = $this->db->get_where('timesheet',array('task_id'=>$t->t_id))->result_array(); 
								foreach($all_task_details as $task){
									$member = $this->db->get_where('account_details',array('user_id'=>$task['user_id']))->row_array();
								?>
							<tr>
								<td><?php echo $member['fullname']; ?></td>
								<td><?php echo $task['timeline_desc']; ?></td>
								<td><?php echo date('d-M-Y',strtotime($task['timeline_date'])); ?></td>
