...
class All_contacts extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->title(lang('all_contacts').' - '.$this->company_name);
        $data['page'] = lang('all_apps');
        $data['sub_page'] = lang('all_contacts');
        $data['form'] = true;
        ...
        $data['all_contacts'] = $this->db->order_by('id','DESC')->get_where('contacts',array('status'=>1))->result_array();
        $this->template
                ->set_layout('users')
    ...
    public function view($company = null)
    {
        $this->load->module('layouts');
        $this->load->library('template');
        $this->template->title(lang('leads').' - '.$this->company_name);
     ...
     public function get_contacts()
    {
                ...
                $all_contacts = $this->db->order_by('contact_name','ASC')->get_where('contacts',array('status'=>1))->result_array();
                                    ...
                                    <ul class="contact-action">
                                        <li class="dropdown">
                                            ...
                                            <ul class="dropdown-menu">
                                                <li><a href="'.base_url().'all_contacts/edit_contact/'.$contact['id'].'" data-toggle="ajaxModal">Edit</a></li>
                                                <li><a href="'.base_url().'all_contacts/delete_contact/'.$contact['id'].'" data-toggle="ajaxModal">Delete</a></li>
                                            </ul>
                                        </li>
                                    </ul>
                                </div>
                                   ...
                                   <div class="pull-left user-img">
                                    ...
                                    <ul class="contact-action">
                                        <li class="dropdown">
                                            <a href="" class="dropdown-toggle action-icon" data-toggle="dropdown" aria-expanded="false"><i class="fa fa-ellipsis-v"></i></a>
                                            <ul class="dropdown-menu">
                                                <li><a href="'.base_url().'all_contacts/edit_contact/'.$contact['id'].'"  data-toggle="ajaxModal" data-target="#edit_contact">Edit</a></li>
                                                <li><a href="'.base_url().'all_contacts/delete_contact/'.$contact['id'].'" data-toggle="ajaxModal" ">Delete</a></li>
                                            </ul>
                                        </li>
                                    </ul>
    ...
    public function search_contact_based_role()
    {
        if ($this->input->post()) {
            if(!empty($_POST['id'])){
                $all_contacts = $this->db->order_by('contact_name','ASC')->get_where('contacts',array('roles'=>$_POST['id']))->result_array();
            }else{
                $all_contacts = $this->db->order_by('contact_name','ASC')->get_where('contacts',array('status'=>1))->result_array();
            }
    ...
    public function search_contact_based_alpha()
    {
        if ($this->input->post()) {
            if(!empty($_POST['name'])){
               $all_contacts = $this->db->select('*')->from('contacts')->where("contact_name LIKE '".$_POST['name']."%'")->order_by('contact_name','ASC')->get()->result_array();
    ...
    public function contact_search()
    {
        if ($this->input->post()) {
            if(!empty($_POST['name'])){
               $roles = $this->db->select('r_id')->from('roles')->where("role LIKE '%".$_POST['name']."%'")->order_by('role','ASC')->get()->result_array();
               $roles = array_column($roles, 'r_id');
               // echo "<pre>";print_r($roles);die;
               $all_contacts = $this->db->select('*')->from('contacts')->where("contact_name LIKE '%".$_POST['name']."%'")->or_where_in('roles',$roles)->order_by('contact_name','ASC')->get()->result_array();
