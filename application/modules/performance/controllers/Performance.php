...
class Performance extends MX_Controller {
    ...
    function index()
    {
        if($this->tank_auth->is_logged_in()) { 
              $this->load->module('layouts');
                ...
                $this->template->title(lang('okr').' - '.$this->company_name); 
    ...
    public function performance_dashboard(){
      ...
      if($this->tank_auth->is_logged_in()) { 
        $this->load->module('layouts');
        $this->load->library('template');
         $this->template->title(lang('performance_dashboard').' - '.$this->company_name); 
        ...
        $data['sub_page']       = lang('performance_dashboard');
        $data['datatables'] = TRUE;
        ...
        if(($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'admin')) {
          ...
          if($performance_status['okr'] ==1){
            $data['okr'] = $this->db->select('IFNULL(ROUND(AVG(k.grade_value),1),0) as key_grade,IFNULL(ROUND(AVG(r.key_gradeval),1),0) as result_grade,IFNULL(ROUND(AVG(k.grade_value+r.key_gradeval),1),0) as peer_grade,o.user_id,o.id,IFNULL(ROUND(AVG(k.progress_value),1),0) as k_progress,IFNULL(ROUND(AVG(r.keyprog_value),1),0) as r_progress,IFNULL(ROUND(AVG(k.progress_value+r.keyprog_value),1),0) as peer_progress')
                ->from('okrdetails o')
                ->where('o.subdomain_id',$this->session->userdata('subdomain_id'))
           ...
           // print_r($data['okr']); exit;
            ...
            $completed_key = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('okr_status !=','Pending')->from('okr_key_results')->get()->num_rows();
            $completed_result = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('key_status !=','Pending')->from('okr_results')->get()->num_rows();
            $pending_key = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('okr_status ','Pending')->from('okr_key_results')->get()->num_rows();
            $pending_result = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('key_status ','Pending')->from('okr_results')->get()->num_rows();
           ...
           $data['completed_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status !=',0)->from('performance_360')->get()->num_rows();
           $data['outstanding_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status',0)->from('performance_360')->get()->num_rows();
          }
          ...
          if($performance_status['smart_goals'] ==1){
           ...
           $data['completed_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status !=',0)->from('smartgoal')->get()->num_rows();
           $data['outstanding_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status',0)->from('smartgoal')->get()->num_rows();
          }
          ...
          if($performance_status['okr'] ==1){
            ...
            $completed_key = $this->db->where('o.subdomain_id',$this->session->userdata('subdomain_id'))->where('k.okr_status !=','Pending')->where('o.lead',$user_id)->from('okrdetails o')->join('okr_key_results k','o.id = k.okrdetailsid','right')->get()->num_rows();

            $completed_result = $this->db->where('o.subdomain_id',$this->session->userdata('subdomain_id'))->where('r.key_status !=','Pending')->where('o.lead',$user_id)->from('okrdetails o')->join('okr_results r','o.id = r.okr_id','right')->get()->num_rows();
            $pending_key = $this->db->where('o.subdomain_id',$this->session->userdata('subdomain_id'))->where('k.okr_status ','Pending')->where('o.lead',$user_id)->from('okrdetails o')->join('okr_key_results k','o.id = k.okrdetailsid','right')->get()->num_rows();
            $pending_result = $this->db->where('o.subdomain_id',$this->session->userdata('subdomain_id'))->where('r.key_status ','Pending')->where('o.lead',$user_id)->from('okrdetails o')->join('okr_results r','o.id = r.okr_id','right')->get()->num_rows();
          ...
          if($performance_status['competency'] ==1){
            $data['performances_360'] = $this->db->select('IFNULL(ROUND(AVG(self_rating),1),0) as self_ratings, IFNULL(ROUND(AVG(rating),1),0) as your_ratings,user_id,IFNULL(ROUND(AVG(self_rating+rating),1),0) as peer_ratings,IFNULL(ROUND(AVG(progress),1),0) as avg_progress')->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('teamlead_id',$user_id)->group_by('user_id')->order_by('id','ASC')->get('performance_360')->result_array(); 
            $data['completed_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status !=',0)->where('teamlead_id',$user_id)->from('performance_360')->get()->num_rows();
            $data['outstanding_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status',0)->where('teamlead_id',$user_id)->from('performance_360')->get()->num_rows();
          }
           if($performance_status['smart_goals'] ==1){
            ...
            $data['completed_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status !=',0)->where('teamlead_id',$user_id)->from('smartgoal')->get()->num_rows();
            $data['outstanding_performance'] = $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'))->where('status ',0)->where('teamlead_id',$user_id)->from('smartgoal')->get()->num_rows();
        }
      }
            ...
            //update objective & result
             $objective_result = array();
              ...
              $okr_result_count = $this->input->post('okr_result_count');
                  ...
                  $objectiveData['subdomain_id']  = $this->session->userdata('subdomain_id');
                  ...
                  foreach($key_results as $obj_key => $result_data){
                      ...
                      $resultData['subdomain_id']  = $this->session->userdata('subdomain_id');
                      ...
                      if($obj_key >= $okr_result_count){                      
                        // foreach ($result_data as $key => $value) {
                          ...
                          $edit_resultData['subdomain_id']  = $this->session->userdata('subdomain_id');
