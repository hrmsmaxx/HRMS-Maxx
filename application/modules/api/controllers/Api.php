...
class Api extends REST_Controller  {
    ...
    function __construct()
    {
       ...
       foreach ($_SERVER as $name => $value)
       {
        ...
        //  $this->apiKey = '46510382';
         $this->apiKey = config_item('apikey_tokbox');
        // $this->apiSecret = '5bc49125930c6d46f74659b58d9d371770393c64';
...
public function profile_post(){
   ...
   if($this->is_valid == TRUE)   {
    ...
    // print_r($result); exit;
    $result['avatar'] = base_url().'assets/avatar/'.$result['employee_details']['avatar'];
...
public function basic_info_post(){
   ...
   if($this->is_valid == TRUE)   {
    ...
    if(!empty($inputs['fullname']) && !empty($inputs['phone'])){
...
public function education_info_post(){
   ...
   if($this->is_valid == TRUE)   {
    ...
    $inputs = $this->post();
    // print_r($inputs['education_info']); exit;
...
public function forgot_password_post(){
    ...
    if(!empty($inputs['username']) && !empty($inputs['subdomain_id'])) {
...
public function projects_details_get(){
   ...
   if($this->is_valid == TRUE)   {
    ...
    $data['saved_item']=$this->db->get_where('items_saved',array('subdomain_id'=>$res['subdomain_id']))->result();

    $data['items']=$this->db->group_by('item_name')->get_where('items',array('subdomain_id'=>$res['subdomain_id']))->result();



    $client=$this->db->where(array('is_lead' => 0,'co_id >'=> 0,'subdomain_id'=>$res['subdomain_id']))->order_by('company_name','ASC')->get('companies')->result_array();
    if(!empty($client))
    {
        ...
        $data['clients']=$clients;
            ...
            $lead=$this->db->where(array('U.role_id !='=>2,'U.role_id !='=>1,'U.banned'=> 0,'subdomain_id'=>$res['subdomain_id']))->get()->result_array();
    ...
    $project=$this->db->get_where('projects',array('subdomain_id'=>$res['subdomain_id']))->result_array();
    if(!empty($project))
    {
...
public function reporting_officer_post()
{
    ...
    if(!empty($inputs['dept_id']) && !empty($inputs['des_id'])){
       ...
       $r = $this->db->get_where('designation',array('id'=>$des_id,'subdomain_id'=>$subdomain_id))->row_array();
       // print_r($r); exit;
        
       if(!empty($r)){
        $grade = $r['grade'];
        $grade_details = $this->db->get_where('grades',array('grade_id'=>$grade))->row_array();
        $all_grades = $this->db->get_where('designation',array('grade <='=>$grade,'subdomain_id'=>$subdomain_id))->result_array();
        $all_users = array();
...
public function add_salary_post()
{
                        ...
                        if($payslips != ''){
                          ...
                          if($salaries != ''){
                            ...
                            if ($payslips['id']) {
                                ...
                                $form_data['basic']             = $pays['addtion||basic_pay'];
                                ...
                                $form_data['deduction_tds']     = $pays['deduction||TDS'];

                                $form_data['deduction_esi']     = $pays['deduction||ESI'];

                                $form_data['deduction_pf']      = $pays['deduction||PF'];

                                $form_data['deduction_leave']   = $pays['deduction||leave'];
    ...
    public function all_users_post()
    {
         if($this->is_valid == TRUE)   
         {
            ...
            {   
                $results = $this->api->all_users($res['user_id'],$inputs['page'],$inputs['subdomain_id']);
            }else{
                $results = $this->api->all_users($res['user_id'],'page',$inputs['subdomain_id']);
            }
                if(!empty($results)){
                        $response['status_code'] = 1;
                        $response['message'] = $this->success;
        ...
        if($this->is_valid == TRUE)   
        {
            ...
            if($res['role_id'] == 1 || $res['role_id'] == 4)
            {
                ...
                foreach($result_list as $expense)
                {
                    ...
                    $expenses_list[$i]['category']= $expense['item_name'];
...
public function one_to_one_chat_messages_post()
    ...
    public function all_payments_post()
    {
        if($this->is_valid == TRUE)    
        {
            ...
            if(($res['role_id'] == 4) || ($res['role_id'] == 1) || ($res['role_id'] == 2))
            {
                // echo "true"; exit;
                $payments_all = array();
                $all_payments = $this->api->getAllPayments($res['subdomain_id']);
    ...
    public function dashboard_count_post()
    {
        if($this->is_valid == TRUE)    
        {
            ...
            {
                $all_projects_admin = $this->api->get_project_counts('admin','',$res['subdomain_id']);
                $all_clients_admin = $this->api->get_clients_counts(2,$res['subdomain_id']);
                $all_companies_admin = $this->api->get_company_counts($res['subdomain_id']);
                $all_staff_admin = $this->api->get_clients_counts(3,$res['subdomain_id']);
                $all_invoice_admin = $this->api->get_invoice_counts('admin','',$res['subdomain_id']);
                $result = array(
                    'project_counts' => count($all_projects_admin),
                    'client_counts' => count($all_companies_admin),
                    'staff_counts' => count($all_staff_admin),
                    'invoice_counts' => count($all_invoice_admin)
                );
            }else if($res['role_id'] == 2){   // client
                $all_projects_client = $this->api->get_project_counts('client',$res['user_id'],$res['subdomain_id']);
                $all_invoice_client = $this->api->get_invoice_counts('client',$res['user_id'],$res['subdomain_id']);
                $all_estimate_client = $this->api->get_estimate_counts($res['user_id'],$res['subdomain_id']);
                $result = array(
                ...
                $all_projects_staff = $this->api->get_project_counts('staff',$res['user_id'],$res['subdomain_id']);
    ...
    public function attendance_details_post()
    {
            ...
            $system_settings = $this->db->get_where('subdomin_system_settings',array('subdomain_id'=>$subdomain_id))->row_array();
    ...
    public function create_attendance_post()
    {
        ...
        if($this->is_valid == TRUE)   {
            ...
            $subdomain_id = $recordss['subdomain_id'];
                     ...
                     if(isset($inputs['attendance_year']) && !empty($inputs['attendance_year']))
                    {
                     ...
                     foreach ($results as $rows) {
                      ...
                      else
                      {
                       ...
                       $row['later_entry_hours']=  (!empty($later_entry_hours))?$later_entry_hours.' hrs':'-';
    ...
    {
        $this->db->select('id as user_id, role_id, subdomain_id');
    ...
    public function run_payroll_get()
    {
         ...
         if($this->is_valid == TRUE)   {
            ...
            //  echo $user_id; exit;
             ...
             $pay_slip_details = array();
                ...
                $total_leaves =  $this->db->get_where('user_leaves',array('user_id'=>
                $user_id,'status'=>1))->row_array();
     ...
     public function change_username_post(){
        ...
        if($this->is_valid == TRUE)   {
            ...
            if(!empty($inputs['user_id']) && !empty($inputs['username']) && !empty($inputs['subdomain_id'])){
    ...
    public function incidents_post(){
        ...
        if($this->is_valid == TRUE)   {
            ...
            if(!empty($inputs['subdomain_id'])){
                $result = $this->api->incidents($inputs['subdomain_id']);    
    ...
    public function punch_groups_post(){
        ...
        if($this->is_valid == TRUE)   {
            ...
            $response['data'] = $data;
            $inputs = $this->post();
            $recordss =  $this->api->get_role_and_userid($this->token);
            if(!empty($recordss)){                  
                $user_id = $recordss['user_id'];
                $subdomain_id = $recordss['subdomain_id'];
                if(!empty($user_id)){
                    $result =  $this->api->punch_group_details($user_id);   
    ...
    public function upload_file_categories_post(){
        ...
        if($this->is_valid == TRUE)   {
            ...
            if(!empty($inputs['user_id'])){
                $result = $this->api->document_types($inputs);    
     ...
     public function all_file_attachments_post(){
        ...
        if($this->is_valid == TRUE)   {
            ...
            if(!empty($inputs['user_id'])){
                $result = $this->api->all_file_attachments($inputs);    

                if(!empty($result)){
                    $uploaddir = 'assets/uploads/profile_attachments/';
                    $response['attach_file_path'] = base_url().$uploaddir;
                    $response['status_code'] = 1;
                    $response['message'] = $this->success;
                    $response['data'] = $result;
                }else{
    ...
    function add_file_attachments_post()
    {
        if($this->is_valid == TRUE)   {
           ...
           // echo "<pre>"; print_r($_POST); exit();
            ...
            if(!empty($inputs['user_id']) && !empty($inputs['doc_type'])){
                $uploaddir = 'assets/uploads/profile_attachments/';
                ...
                if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadfile)) {
                    $res = array(
                        ...
                        'is_expired' => isset($inputs['is_expired'])?$inputs['is_expired']:'0',
                        'expired_date' => ($inputs['expired_date'] !='')?date('Y-m-d',strtotime($inputs['expired_date'])):'',
                        'before_expired_day' => ($inputs['before_expired_day'] != '')?$inputs['before_expired_day']: '0',
                    ...
                    $result = $this->db->insert('dgt_user_files',$res);
                    // echo $this->db->last_query(); exit();
                    $insert_id = $this->db->insert_id();
                    if($result){
                        $response['status_code'] = 1;
