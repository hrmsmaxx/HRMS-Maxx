...
class Appuser extends REST_Controller  {
    ...
    function __construct()
    {
       ...
       // echo "<pre>"; print_r($_SERVER); exit;
        // $header =  apache_request_headers();// Get Header Data
    ...
    public function login_post()
    {
       ...
       if($this->is_valid == TRUE)   {
            ...
            if(!empty($inputs['username']) && !empty($inputs['password']) && !empty($inputs['device_id']) && !empty($inputs['subdomain_id'])){
                ...
                $result = $this->user->is_valid_login($inputs); 
                  ...
                  if(!empty($result)){
                    $check_device = $this->user->check_device($result['id']);
                        ...
                        $user_id = $result['id'];
                        $punch_group_details = $this->user->punch_group_details($user_id);
                        $incidents = $this->user->incidents($inputs['subdomain_id']);
                        $response['punch_group_details'] = $punch_group_details;
     ...
     public function create_employee_post()
    {
       ...
       if($this->is_valid == TRUE)   {
            ...
            if(!empty($inputs['username']) && !empty($inputs['password'])&& !empty($inputs['email'])&& !empty($inputs['fullname'])  && !empty($inputs['phone']) && !empty($inputs['emp_doj'])){
...
// exit;
    ...
    public function punch_in_post()
    {
        if($this->is_valid == TRUE)   {
            ...
            $response['message'] = 'Required input missing';
            $response['data'] = $data;
            $recordss =  $this->user->get_role_and_userid($this->token);
            ...
            date_default_timezone_set($time_zone);
                 ...
                 $strtotime = strtotime(date('Y-m-d H:i'));
                  ...
                  $inputs = $this->post();
                if(isset($inputs['incident_id'])){
                    $record = $this->user->get_attendance_data($this->token,$inputs['incident_id']);                                
        ...
        if($this->is_valid == TRUE)   {
            ...
            date_default_timezone_set($time_zone);
                  ...
                  $a_cout     = date('H:i',$strtotime); 
                    $inputs = $this->post();
                if(isset($inputs['incident_id'])){
                   $record = $this->user->get_punchout_attendance_data($this->token,$inputs['incident_id']);                
                    if($record){
    ...
    public function app_colorcode_post()
   {
       $result = array(
           ...
           'secondry_color'    =>  '#ffa500',
           'black_logo'        =>  base_url().'assets/images/black-logo.png',
           'full_white_logo'   =>  base_url().'assets/images/white-logo.png',
           'white_logo'        =>  base_url().'assets/images/white-logo.png'
       );
       $response['status_code'] = 1;
       $response['message'] = 'SUCCESS';
       $response['data'] = $result;
   ...
   public function face_punch_post()
   {
    ...
    function check_workspace_post()
    {
        //echo 123;exit;
        $inputs = $this->input->post();
        $workspace = $inputs['workspace'];
        if(!empty($workspace)){
            $workspace_check = $this->db->get_where('subscribers',array('workspace'=>$workspace))->row_array();

            if(count($workspace_check) != 0){
                $str=$_SERVER['HTTP_HOST'];
                $servername = ltrim($str,'demo');
                $workspace_check['redirect_url'] =$workspace.''.$servername;
                 
                $response['status_code'] = 1;
                $response['message'] = 'SUCCESS';
                $response['data'] = $workspace_check;
            }else{
                $response['status_code'] = 0;
                $response['message'] = 'Invalid Workspace';
            }
        }else{
            $response['status_code'] = 0;
            $response['message'] = 'Required inputs missing';
