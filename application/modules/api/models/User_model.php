...
class User_model extends CI_Model
{
	...
	public function is_valid_token($token)
	{
	 ...
	 public function is_valid_login($input)
	 {
		...
		if($hasher->CheckPassword($password, $check_password)){
		 	...
		 	$this->db->select('U.id,AD.fullname,U.unique_code,U.email,U.role_id,U.subdomain_id');
		 	$this->db->from($this->user);
		 	$this->db->join($this->account_details, 'AD.user_id = U.id', 'left');
		 	$this->db->join($this->companies, 'C.co_id = AD.company', 'left');

		 	$where = array('activated'=>1,'banned'=>0,'U.subdomain_id'=>$subdomain_id);
		 	$this->db->where($where);
	 	...
	 	if(!empty($records)){
	 		// echo 'Test '.$records['currency']; exit;
	...
	{		
			...
			if(!empty($record)){
				$role    = $record['role_id'];
			 	if($role == 1 || $role == 4){
			 		...
			 		$user['subdomain_id'] = $subdomain_id;
			 		$user['activated'] = 1;
			...
			if(!empty($record)){
				$role    = $record['role_id'];
				 ...
				 else
				 {
				 	...
				 	$projects['subdomain_id']=$record['subdomain_id'];
                 ...
                 if(isset($inputs['due_date']) && !empty($inputs['due_date']))
                {
					...
					// $this->db->insert('dgt_projects', $projects); 
	 ...
	 public function create_client($inputs,$token)
	 {
		...
		$records = array();
            ...
            $_POST['subdomain_id'] = $record['subdomain_id'];
	 ...
	 public function edit_client($inputs,$token)
	 {
		...
		$records = array();
            ...
            foreach ($inputs as $key => &$value) {
                ...
                $inputs['company_website'] = prep_url($inputs['company_website']);
	  ...
	  public function create_invoice($inputs,$token)
	 {
		...
		$records = array();
			if(!empty($record)){
                ...
                $invoices['subdomain_id'] = $record['subdomain_id'];
	...
	public function create_estimate($inputs,$token)
	 {
		...
		$records = array();
			if(!empty($record)){
                ...
                $estimates['subdomain_id'] = $record['subdomain_id'];
                unset($_POST['files']);
   ...
   function crypto_rand_secure($min, $max) {
	...
	public function get_attendance_data($token,$incident_id)
	{
		$records =  $this->get_role_and_userid($token);
			...
			if(!empty($records)){
				...
				$subdomain_id    = $records['subdomain_id'];
				...
				$strtotime = strtotime(date('Y-m-d H:i'));
                  ...
                  if(empty($record)){
        			...
        			$inputs['subdomain_id'] = $subdomain_id;
                      ...
                      if($record_day[$a_day]['punch_in'] ==''){
                        ...
                        $record_day[$a_day]['day'] = 1;
                     	$record_day[$a_day]['scheduling_id'] = $scheduling_id;
       					...
       					$record_day[$a_day]['punchin_workcode'] = $incident_id;
                      }
                      
                      if($total_records == 1 && empty($current_day['punch_out'])){
                        
                        $current_days = array('day'=>1,'punch_in'=>$a_cin,'punch_out'=>'','scheduling_id'=>$scheduling_id,'project_id'=>$project_id,'punchin_workcode'=>$incident_id);
                        $month_days_in_out_record[$a_day][0] = $current_days;
                      }else{
                        
                        if(!empty($current_day['punch_in']) && !empty($current_day['punch_out']))
                        {
                          $current_days[$total_records] =array('day'=>1,'punch_in'=>$a_cin,'punch_out'=>'','scheduling_id'=>$scheduling_id,'project_id'=>$project_id,'punchin_workcode'=>$incident_id);
	...
	public function get_punchout_attendance_data($token,$incident_id)
	{
		$records =  $this->get_role_and_userid($token);
      ...
      if(!empty($record['month_days'])){
        ...
        if(!empty($record_day[$a_day])){
            ...
            $record_day[$a_day]['day'] = 1;
          	$record_day[$a_day]['punchout_workcode'] = $incident_id;
        }
        if($total_records == 1 && empty($current_day['punch_out'])){
            ...
            if(!empty($current_day['punch_in']) && empty($current_day['punch_out']))
            {
               ...
               $current_days[$total_records-1]['punchout_workcode'] = $incident_id;
               $month_days_in_out_record[$a_day] = $current_days;
            } 
          }
