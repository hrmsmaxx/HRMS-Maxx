...
class Api_model extends CI_Model {
	...
	public function employee_list($token,$inputs,$type=1)
	{
		...
		$this->db->where('U.subdomain_id',$record['subdomain_id']);
...
// 	}
    ...
    public function get_employeedetailById($id)
	{
	   ...
	   return $this->db->select('U.*,AD.*,d.deptname,ds.designation,r.fullname as reporting_to')
	            ->from('dgt_users U')
	            ->join('dgt_account_details AD', 'U.id = AD.user_id','LEFT')
	            ->join('dgt_account_details r', 'U.teamlead_id = r.user_id','LEFT')
	            ->join('dgt_designation ds', 'U.designation_id = ds.id','LEFT')
	            ->join('dgt_departments d', 'U.department_id = d.deptid','LEFT')
	            ->where('U.id',$id)
	            ->get()->row_array();
	...
	public function forgot_password($token,$input)
	{
		...
		// $record =  $this->get_role_and_userid($token);
		// $record =  $this->get_role_and_username($username,$subdomain_id);
	    $records = array();
		// if(!empty($record)){
		// 	$role     = $record['role_id'];
		// 	$user_id  = $record['user_id'];
		 	// if($role == 3){
		 		...
		 		$this->db->from($this->user);
		 		$this->db->join($this->account_details, 'AD.user_id = U.id', 'left');
			 	$where = array('activated'=>1,'banned'=>0,'subdomain_id'=>$subdomain_id);
			 	$this->db->where($where);
			 	// $this->db->where('(role_id = 3)');
		...
		if(!empty($record)){
			$role    = $record['role_id'];
		 	if($role == 1 || $role == 3 || $role == 4){
		 		...
		 		$this->db->where('subdomain_id',$record['subdomain_id']);
	...
	public function incidents($subdomain_id)
	{		
		$records = array();
		$this->db->order_by('incident_name', 'ASC');
 		$this->db->where('subdomain_id',$subdomain_id);
 		$this->db->select('*');
 		$records = $this->db->get('dgt_incidents')->result();
		return $records;
	}
	public function document_types($inputs)
	{		
		$records = array();
		$this->db->order_by('type_name', 'ASC');
 		$this->db->where('user_id',$inputs['user_id']);
 		$this->db->select('*');
 		$records = $this->db->get('dgt_document_types')->result();
		return $records;
	}
	public function all_file_attachments($inputs)
	{		
		$records = array();
		$this->db->order_by('user_file_id', 'ASC');
 		$this->db->where('user_id',$inputs['user_id']);
 		$this->db->select('*');
 		$records = $this->db->get('dgt_user_files')->result();
		return $records;
	}
	...
	public function create_department($token,$input)
	{
		...
		if(!empty($record['role_id'])){
			...
			if($role == 1 || $role == 4){
				...
				$where['subdomain_id'] = $record['subdomain_id'];

				$alreay = $this->check_input_exists($this->departments,$where);
				if($alreay==0){
					$this->db->insert($this->department, array('deptname'=>$input['department'],'subdomain_id'=>$record['subdomain_id']));
		...
		public function leaves($token,$input,$type)
	{
		...
		if(!empty($record)){
			...
			$leave_status    = ($input['leave_status'] != '')?$input['leave_status']:'';
		 		...
		 		$this->db->join('common_leave_types CL', 'CL.leave_id = UL.leave_type', 'left');
	...
	public function get_role_and_username($username,$subdomain_id)
	{
		$this->db->select('id as user_id, role_id');
		return $this->db->get_where($this->user,array('username'=>$username,'activated'=>1,'subdomain_id'=>$subdomain_id))->row_array();
	}
	public function check_username($inputs)
	{
		$this->db->select('id as user_id, role_id');
		return $this->db->get_where($this->user,array('username'=>$inputs['username'],'activated'=>1,'subdomain_id'=>$inputs['subdomain_id']))->row_array();
	}
...
// 		return $this->db->get_where($this->salary,array('user_id'=>$user_id))->row_array();
		return $this->db->get_where('bank_statutory',array('user_id'=>$user_id))->row_array();
// 		echo $this->db->last_query();
	...
	public function all_users($user_id,$page,$subdomain_id=NULL)
	{
		...
		$this->db->where('U.subdomain_id',$subdomain_id);	
		}
	...
	public function project_listByUserId($token,$inputs,$type,$user_id)
	{
		...
		$this->db->where('PJ.subdomain_id', $record['subdomain_id']);
	...
	{
		return $this->db->select('C.*,AD.fullname,AD.user_id,AD.id')
				->from($this->companies)
				->join($this->account_details,'AD.id = C.primary_contact','left')
				->where('C.is_lead','0')
				->where('C.co_id',$co_id)
	...
	public function get_all_estimate($token,$inputs,$type=1)
	{
		        ...
		        $res =  $this->get_role_and_userid($token);
					...
					$this->db->join($this->account_details,'AD.user_id = U.id',LEFT);
					$this->db->where('ES.subdomain_id',$res['subdomain_id']);
	...
	public function get_all_invoices($token,$inputs,$type=1)
	{
		        ...
		        $res =  $this->get_role_and_userid($token);
					...
					$this->db->join($this->account_details,'AD.user_id = U.id',LEFT);
					$this->db->where('IN.subdomain_id',$res['subdomain_id']);
	...
	public function get_all_expenses($token,$inputs,$type=1)
	{
		        $page    = !(empty($inputs['page']))?$inputs['page']:1;
		        $res =  $this->get_role_and_userid($token);
	            $this->db->select('EX.*,PJ.project_title,IS.item_name'); 
				...
				$this->db->join('items_saved IS','IS.item_id = EX.category',LEFT);
				$this->db->where('EX.subdomain_id',$res['subdomain_id']);
				 if($type == 1){
			  		return $this->db->count_all_results();	
				}else{
					$page = !empty($inputs['page'])?$inputs['page']:'';
	...
	public function getAllPayments($subdomain_id)
	{
		return $this->db->select('*,IN.reference_no,IN.date_saved,IN.inv_id')
				 ->from($this->payments)
				 ->join($this->companies,'PY.paid_by = C.co_id',LEFT)
				 ->join($this->payment_methods,'PY.payment_method = PYM.method_id',LEFT)
				 ->join($this->invoice, 'PY.invoice = IN.inv_id',LEFT)
				 ->where('PY.inv_deleted','No')
				 ->where('PY.subdomain_id',$subdomain_id)
				 ->get()->result_array();
		// return $this->db->get_where($this->payments,array('PY.inv_deleted'=>'No'))->result_array();
	}


	public function get_project_counts($role,$id,$subdomain_id)
	{
		if($role == 'admin')
		{
			$res = $this->db->get_where('dgt_projects',array('status'=>'Active','subdomain_id'=>$subdomain_id))->result_array();
		}else if($role == 'staff'){
			$res = $this->db->get_where('dgt_assign_projects',array('assigned_user'=>$id))->result_array();
		}else if($role == 'client'){
			$res = $this->db->get_where('dgt_projects',array('status'=>'Active','client'=>$id,'subdomain_id'=>$subdomain_id))->result_array();
		}
		return $res;
	}

	public function get_clients_counts($role_id,$subdomain_id)
	{
		return $this->db->get_where('dgt_users',array('role_id'=>$role_id,'activated'=>1,'banned'=>0,'subdomain_id'=>$subdomain_id))->result_array();
	}
	
	public function get_company_counts($subdomain_id)
	{
		return $this->db->get_where($this->companies,array('subdomain_id'=>$subdomain_id))->result_array();
	}
	...
	public function get_invoice_counts($role,$id,$subdomain_id)
	{
		if($role == 'admin')
		{
			return $this->db->get_where('dgt_invoices',array('subdomain_id'=>$subdomain_id))->result_array();
		}else if($role == 'client'){
			return $this->db->get_where('dgt_invoices',array('client'=>$id,'subdomain_id'=>$subdomain_id))->result_array();
		}
	}
    ...
    {   
 	...
 	Public function punch_group_details($user_id)
	{	   
	  	return $this->db->select("dgt_punch_groups.*,dgt_locations.*,dgt_locations.id as location_id")
