...
class Interview_questions extends MX_Controller
{
    ...
    {   
       ...
       // echo $this->db->last_query();exit; 
        ...
        $where['question_category.subdomain_id'] =$this->session->userdata('subdomain_id');
        $data['categories'] = $this->Jobs_model->select_question_category($where);
        $data['questions_list'] = $this->Jobs_model->select_question_list(array('interview_questions.deleted_at'=>NULL,'jobs.subdomain_id'=>$this->session->userdata('subdomain_id')));
        //echo $this->db->last_query();exit;      
        $this->template
                ->set_layout('users')
    ...
    function category(){
        ...
        $data['departments'] = $this->Jobs_model->select('departments',array('subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch job department*/
         if($this->uri->segment(3)!=""){
        $data['category'] = $category = $this->Jobs_model->select_row_array('question_category',array('id'=>$this->uri->segment(3)));
        $data['jobs_list'] = $this->Jobs_model->select('jobs',array('department_id'=>$category['dept_id'],'subdomain_id'=>$this->session->userdata('subdomain_id')));
    ...
    function jobs_list(){
        $jobs_list = $this->Jobs_model->select_row('jobs',array('department_id'=>$this->input->post('department'),'subdomain_id'=>$this->session->userdata('subdomain_id')));
	...
	public function edit_questions()
    {
        ...
        $data['departments'] = $this->Jobs_model->select('departments',array('subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch  department*/
        $data['jobs_list'] = $this->Jobs_model->select('jobs',array('id'=>$question['job_id'],'subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch job */
