...
<div class="content">
	...
	<div class="row">
		...
		<div class="alert alert-success alert-dismissible">
		    ...
		    <strong><?php echo lang('done'); ?>!</strong> <?php echo lang('subscribe_success'); ?>
		 </div>
		...
		<div class="alert alert-danger alert-dismissible">
		    <button type="button" class="close" data-dismiss="alert">&times;</button>
		    <strong><?php echo lang('error'); ?>!</strong> <?php echo lang('please_try_again'); ?>
		</div>
...
<?php $attributes = array('id' => 'stripeFormSub'); echo form_open(base_url().'stripepay/sub_authenticate',$attributes); ?>
	...
	<input type="hidden" id="stripeamountsub" name="stripeamountsub"/>
		...
		<div class="col-md-8 text-right m-b-30">
			<a href="javascript:void(0)" class="btn btn-primary pull-right rounded m-l-5 m-b-10" data-toggle="modal" data-target="#add_new_user"><i class="fa fa-plus"></i> <?=lang('new_user')?></a>
					...
					<div class="row filter-row">
						<div class="col-sm-3 col-xs-6">  
							<div class="form-group form-focus">
								...
								<label id="username_error" class="error display-none" for="username"><?php echo lang('username_should_not_be_empty'); ?></label>
							</div>
						</div>
						...
						<div class="col-sm-3 col-xs-6"> 
							<div class="form-group form-focus select-focus">
								...
								<select class="select floating form-control" id="user_role" name="user_role"> 
									<option value="" selected="selected"><?php echo lang('all_rols'); ?></option>
									<?php
										$roles = User::get_roles();
										// echo $this->session->userdata('subdomain_id');
								...
								<label id="user_role_error" class="error display-none" for="user_role"><?php echo lang('please_select_a_role'); ?></label>
							</div>
						</div>
						...
						<div class="col-sm-3 col-xs-6"> 
							<div class="form-group form-focus select-focus">
								...
								<select class="select floating form-control" id="company" name="company"> 
									<option value="" selected="selected"><?php echo lang('all_companies'); ?></option>
									<?php if(!empty($companies)){ ?>
									<?php foreach ($companies as $company) { 
											if($company->subdomain_id == $this->session->userdata('subdomain_id')){
										?>
										<option value="<?php echo $company->co_id; ?>"><?php echo $company->company_name; ?></option>
									<?php  } } ?>
									<?php } ?>
								</select>
								<label id="company_error" class="error display-none" for="company"><?php echo lang('please_select_a_company'); ?></label>
							</div>
						</div>
						...
						<div class="col-sm-3 col-xs-6">  
							<a href="javascript:void(0)" id="users_search_btn" class="btn btn-success btn-block"> <?php echo lang('search'); ?> </a>  
						</div>  
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="table-responsive">
				<table id="table-users" class="table table-hover custom-table">
					<thead>
						<tr>
	...
	<div id="add_new_user" class="modal custom-modal fade" role="dialog" style="display: none;">
		<div class="modal-dialog">
			<div class="modal-content modal-lg">
				<div class="modal-header">
					<h4 class="modal-title"><?php echo lang('add_new_user'); ?></h4>
					<button type="button" class="close" data-dismiss="modal">&times;</button>
				</div>
				<div class="modal-body m-b-0">
					<?php $attributes = array('id'=>'addNewUser'); echo form_open(base_url().'auth/register_user',$attributes); ?>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" class="form-control" value="" placeholder="<?=lang('eg')?> <?=lang('placeholder_first_name')?>" name="first_name" autocomplete="off">
							</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label><?=lang('last_name')?> <span class="text-danger">*</span></label>
									<input type="text" class="form-control" placeholder="<?=lang('eg')?> <?=lang('placeholder_last_name')?>" name="last_name" autocomplete="off" >
								</div>
							</div>
						</div>
						<div class="row">							
						...
						<div class="row">
							<div class="col-sm-6">
								<div class="form-group">
									...
									<input type="text" class="form-control datetimepicker" data-date-format="yyyy-mm-dd" value="" name="dob" autocomplete="off">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label><?=lang('phone')?> <span class="text-danger">*</span></label>
									<input type="text" class="form-control telephone" value="<?=set_value('phone')?>" name="phone" placeholder="<?=lang('eg')?> <?=lang('user_placeholder_phone')?>">
								</div>
							</div>
						</div>
						<div class="row">
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<select name="role" class="form-control" id="roles">
										<option value="" selected><?php echo lang('select_role'); ?></option>
										<?php foreach (User::get_roles() as $r) {
											if ($r->role =='admin' || $r->role =='client') {
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<select class="select2-option form-control" style="width:100%;" name="company" id="add_company">
										<option value="" disabled selected="selected"><?php echo lang('select_companies'); ?></option>
										<?php if(!empty($companies)){ ?>
										<?php foreach ($companies as $company) { 
												if($company->subdomain_id == $this->session->userdata('subdomain_id')){
											?>
						...
						<div class="submit-section">
							<button class="btn btn-primary submit-btn" id="user_add_new"><?=lang('register_user')?></button>
						</div>
					</form>
					</div>
				</div>
		...
		<div class="modal-dialog">
			...
			<div class="modal-content modal-lg">
				<div class="modal-header">
					<h4 class="modal-title"><?php echo lang('user_subscription'); ?></h4>
				</div>
				<div class="modal-body m-b-30">
						...
						<form method="post" id="paypal_form">
							<div class="col-md-12">
								<span id="error_sub" style="color: red;display: none;"> *<?php echo lang('please_fill_required_filed'); ?></span>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label><?php echo lang('choose_no_of_users'); ?><span class="text-danger">*</span></label>
									<input type="text" name="user_count" id="user_count" class="form-control" placeholder="Users count">
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label><?php echo lang('subscription_amount'); ?><span class="text-danger">*</span></label>
									<input type="text" name="sub_amount" id="sub_amount" placeholder="$11" value="<?=set_value('sub_amount')?>" class="form-control" disabled>
								</div>
							</div>
							<div class="col-md-6">
								<div class="form-group">
									<label><?php echo lang('payment_type'); ?><span class="text-danger">*</span></label>
									<div class="btn-group" id="status" data-toggle="buttons">
						              <label class="btn btn-default btn-on btn-lg active">
						              <input type="radio" class="payment_type_option" value="stripe" name="payment_type" id="payment_type" checked="checked"><?php echo lang('stripe'); ?></label>
						              <label class="btn btn-default btn-off btn-lg">
						              <input type="radio" class="payment_type_option" value="paypal" name="payment_type" id="payment_type"><?php echo lang('paypal'); ?></label>
						            </div>
								</div>
							</div>
							<div class="m-t-20 text-center">
								<a href="javascript:void(0);" id="user_subscripe" class="btn btn-primary"><?php echo lang('subscribe'); ?></a>
								<button id="paypal_subscription"  class="btn btn-primary" style="display: none;"><?php echo lang('subscribe'); ?> </button>
							</div>
					</form>
						</div>
				</div>
...
<?php
 ...
 $d_company = '<optgroup label="'.lang('default_company').'" style="display: none;"><option value="-">'.$this->company_name.'</option></optgroup>';
	$other_companies ='';									
	...
	$(document).ready(function(){
		$('input[type=radio][name=payment_type]').change(function() {
	 	...
	 	$('#roles').change(function(){
        	var company=$("#roles option:selected").text();        	
        	if(company == 'Admin'){
