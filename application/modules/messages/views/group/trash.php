...
<li class="<?php if($msg->status == 'Unread'){echo  "unread"; }?>">
	<div class="trash-link">
		...
		<span class="perm-delete"> 
			<a href="<?=base_url()?>messages/restore/<?=$msg->msg_id?>" title="<?=lang('restore')?>"><i class="fa fa-retweet text-primary"></i></a>
			<a href="<?=base_url()?>messages/remove/<?=$msg->msg_id?>" title="<?=lang('permanent_delete')?>"><i class="fa fa-times text-danger"></i></a>
		</span>
	</div>
</li>
<?php } ?>
