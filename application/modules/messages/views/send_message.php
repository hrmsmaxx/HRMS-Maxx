<div class="content">
	<div class="row">
		...
		<div class="col-sm-4  text-right m-b-20">     
	          <a class="btn back-btn" href="<?=base_url()?>messages"><i class="fa fa-chevron-left"></i> <?php echo lang('back');?></a>
	      </div>
	</div>
	<div class="row">
									...
									<?php echo form_open(base_url().'messages/search/'); ?>
										<div class="input-group">
											<input type="text" class="form-control" name="keyword" placeholder="<?=lang('keyword')?>">
												<span class="input-group-btn"> <button class="btn btn-md btn-default" type="submit"><?=lang('go')?>!</button>
												</span>
										</div>
									</form>
								</div>
									...
									<div class="">
										...
										<div class="form-group">
											...
											<div class="col-lg-9">
												<div class="m-b">
													<select class="select2-option form-control" id="username_select" multiple="multiple" style="width:260px" name="user_to[]" >
														...
														<optgroup label="<?=lang('administrators')?>">
															<?php foreach (User::admin_list() as $admin): if($admin->subdomain_id == $this->session->userdata('subdomain_id')){ ?>
																	<option value="<?=$admin->id?>"><?=ucfirst(User::displayName($admin->id))?></option>
																<?php } endforeach; ?>
														</optgroup>
														<optgroup label="<?=lang('staff')?>">
															<?php foreach (User::staff_list() as $s): if($s->subdomain_id == $this->session->userdata('subdomain_id')){?>
																<option value="<?=$s->id?>">
																<?=ucfirst(User::displayName($s->id))?></option>
															<?php } endforeach; ?>
														</optgroup>
														<optgroup label="<?=lang('clients')?>">
															<?php foreach (User::user_list() as $client): if($client->subdomain_id == $this->session->userdata('subdomain_id')){ ?>
												...
												<div class="row">
												<div class="col-md-6">
												<label id="username_message_error" class="error display-none" style="position:inherit;top:0;font-size:15px;"><?=lang('please_select_username')?></label>
												</div>
												</div>
											</div>
										</div>
										<div class="form-group">
											<label class="col-lg-3 control-label"><?=lang('messages')?> <span class="text-danger">*</span></label>
											<div class="col-lg-9">
												<textarea name="message"  class="form-control foeditor-send-message"></textarea>
												<div class="row">
												<div class="col-md-6">
												<label id="send_message_error" class="error display-none" style="position:inherit;top:0;font-size:15px;"><?=lang('msg_not_empty')?></label>
												</div>
												</div>
											</div>
										</div>
