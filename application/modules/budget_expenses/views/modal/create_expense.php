...
<?php $currency = $this->db->get('currency')->result_array(); ?>
				...
				<div class="form-group">
					...
					<div class="col-lg-4">
						<select name="currency_symbol" class="form-control">
							...
							<option value="<?=$cur->symbol?> - <?=$cur->code?>" <?=(config_item('default_currency') == $cur->code ? ' selected="selected"' : '')?>><?=$cur->symbol?> - <?=$cur->name?></option>
	                        <?php endforeach; ?>
	                    </select>
                    </div>
				</div>
