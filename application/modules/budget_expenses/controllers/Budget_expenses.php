...
class Budget_expenses extends MX_Controller {
...

	{
		...
		$this->template->title(lang('budget_expenses').' - '.$this->company_name);

		$data['page'] = lang('accounting');

		$data['sub_page'] = lang('budget_expenses');
		...
		$data['categories'] = $this->db->get_where('budget_category',array('subdomain_id'=>$this->session->userdata('subdomain_id')))->result_array();
