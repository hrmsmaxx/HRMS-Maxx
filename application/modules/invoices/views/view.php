...
<div class="content">
	<div class="row">
        ...
        <div class="col-sm-4  text-right m-b-20">     
              <a class="btn back-btn" href="<?=base_url()?>invoices"><i class="fa fa-chevron-left"></i> <?php echo lang('back');?></a>
          </div>
	</div>
                <div class="card-box hidden-print">
                    <div class="row">
						<div class="col-md-12 m-t-8 m-l-2">
                            ...
                            <a href="#" class="m-b-5 btn btn-sm btn-primary" onClick="window.print();"><i class="fa fa-print"></i></a>
                            <?php if (User::is_admin() || User::perm_allowed(User::get_id(),'edit_all_invoices')) : ?>
							<a href="<?= base_url() ?>invoices/items/insert/<?=$inv->inv_id ?>" title="<?= lang('item_quick_add') ?>" class="m-b-5 btn btn-sm btn-info" data-toggle="ajaxModal">
								<i class="fa fa-list-alt text-white"></i> <?= lang('items') ?>
							</a>
							<?php if ($inv->show_client == 'Yes') { ?>
							<a class="m-b-5 btn btn-sm btn-success" href="<?= base_url() ?>invoices/hide/<?=$inv->inv_id ?>" data-toggle="tooltip" data-original-title="<?=lang('hide_to_client') ?>" data-placement="bottom">
								<i class="fa fa-eye-slash"></i>
							</a>
							<?php } else { ?>
							<a class="m-b-5 btn btn-sm btn-danger" href="<?= base_url() ?>invoices/show/<?= $inv->inv_id ?>" data-toggle="tooltip" data-original-title="<?= lang('show_to_client') ?>" data-placement="bottom">
								<i class="fa fa-eye"></i>
							</a>
							<?php } ?>
                            ...
                            <?php if (Invoice::get_invoice_due_amount($inv->inv_id) > 0) : ?>
                                ...
                                <?php if (User::is_admin() || User::perm_allowed(User::get_id(),'pay_invoice_offline')
                                    ...
                                    <a class="m-b-5 btn btn-sm btn-<?=$theme_color;?>"
                                ...
                                <?php } else { ?>
                                    ...
                                    <?php if ($inv->allow_stripe == 'Yes') : ?>
                                        ...
                                        <button id="customButton" class="m-b-5 btn btn-sm btn-<?=$theme_color;?>"><?=lang('via_stripe')?>
                                        </button>
                            ...
                            <?php if ($inv->recurring == 'Yes') { ?>
                                <?php if(User::is_admin() || User::perm_allowed(User::get_id(),'edit_all_invoices')){ ?>
                                    ...
                                    <a class="m-b-5 btn btn-sm btn-warning" href="<?= base_url() ?>invoices/stop_recur/<?=$inv->inv_id?>"
                            ...
                            <?php if (User::is_admin() || User::perm_allowed(User::get_id(),'edit_all_invoices')) { ?>
                                ...
                                <a href="<?= base_url() ?>invoices/edit/<?=$inv->inv_id?>"
                                   class="m-b-5 btn btn-sm btn-success" data-original-title="<?=lang('edit_invoice')?>"
                            ...
                            <?php } if (User::is_admin() || User::perm_allowed(User::get_id(),'delete_invoices')) { ?>
                                ...
                                <a href="<?= base_url() ?>invoices/delete/<?=$inv->inv_id?>" class="m-b-5 btn btn-sm btn-danger"
                            ...
                            <?php if (($this->session->userdata('role_id') == 1) || ($this->session->userdata('role_id') == 4)){ ?>
                                ...
                                <a data-invid="<?= $inv->inv_id ?>" data-target="#clone_invoice<?= $inv->inv_id ?>" data-toggle="modal" class=" btn btn-sm btn-info" title="<?=lang('clone_invoice')?>" ><?=lang('clone')?></a>
                            <?php } ?>
                ...
                <?php else: ?>
                    <div class="alert alert-danger hidden-print">
                        <button type="button" class="close" data-dismiss="alert">×</button> <i class="fa fa-warning"></i>
                        <?=lang('this_invoice_cancelled')?>
                    </div>

                <?php endif; ?>

                    <div class="panel inv-panel m-b-0">
...
<?php } ?>
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-xs-3 col-sm-6">
                                <div style="height: <?=$invoice_logo_height;?>px">
                                    <img class="ie-logo" src="<?= base_url() ?>assets/images/logos/<?= $invoice_logo; ?>" style="height: <?=$invoice_logo_height;?>px">
                                </div>
                            </div>
                        ...
                        <div class="m-t">
                            <div class="row">
                                ...
                                <a href="tel:<?= (config_item('company_phone_' . $l)
                                    ...
                                    <?php endif; ?>
                                            ...
                                            <?php if (config_item('company_fax_'.$l) != '' || $company_fax != '') : ?>
                                                ...
                                                <span class="col-xs-9 no-gutter">
                                <a href="tel:<?= (config_item('company_fax_' . $l) ? config_item('company_fax_' . $l) : $company_fax) ?>">
                                ...
                                <a href="tel:<?= (config_item('company_registration_' . $l) ? config_item('company_registration_' . $l) : $company_registration) ?>">
    ...
    <a href="<?=site_url()?>invoices/add_expenses/<?=$inv->client?>/<?=$inv->inv_id?>" class="btn btn-xs btn-danger" data-toggle="ajaxModal"><?=lang('expenses_available')?></a>
                            ...
                            <span class="col-xs-9 no-gutter">
                                ...
                                <?= (config_item('company_country_' . $l) ? config_item('company_country_' . $l) : $company_country) ?>
                            </span>

                                            <span class="col-xs-3 no-gutter"><?= lang('phone') ?>:</span>
                            <span class="col-xs-9 no-gutter">
                                <a href="tel:<?= (config_item('company_phone_' . $l) ? config_item('company_phone_' . $l) : $company_phone) ?>">
                                    <?= (config_item('company_phone_' . $l) ? config_item('company_phone_' . $l) : $company_phone) ?></a>

                                <?php if (config_item('company_phone_2_'.$l) != '' || $company_phone_2 != '') : ?>
                                    , <a href="tel:<?= (config_item('company_phone_2_' . $l) ? config_item('company_phone_2_' . $l) : $company_phone_2) ?>">
                                        <?= (config_item('company_phone_2_' . $l) ? config_item('company_phone_2_' . $l) : $company_phone_2) ?></a>
                                <?php endif; ?>
                            </span>
                                            ...
                                            <?php if (config_item('company_fax_'.$l) != '' || $company_fax != '') : ?>
                                                ...
                                                <span class="col-xs-9 no-gutter">
                                <a href="tel:<?= (config_item('company_fax_' . $l) ? config_item('company_fax_' . $l) : $company_fax) ?>">
                                    <?= (config_item('company_fax_' . $l) ? config_item('company_fax_' . $l) : $company_fax) ?></a>
                            </span>
                                            <?php endif; ?>
                                            ...
                                            <?php if (config_item('company_registration_'.$l) != '' || $company_registration != '') : ?>
                                                ...
                                                <span class="col-xs-9 no-gutter">
                                <a href="tel:<?= (config_item('company_registration_' . $l) ? config_item('company_registration_' . $l) : $company_registration) ?>">
                                    <?= (config_item('company_registration_' . $l) ? config_item('company_registration_' . $l) : $company_registration) ?></a>
                            </span>
                                            <?php endif; ?>
                                            <?php if (config_item('company_vat_'.$l) != '' || $company_vat != '') : ?>
                                            <span class="col-xs-3 no-gutter"><?=lang('company_vat')?>:</span>
                                            <span class="col-xs-9 no-gutter">
                            <?= (config_item('company_vat_' . $l) ? config_item('company_vat_' . $l) : $company_vat) ?><br>
                            <span>
                        <?php endif; ?>
                                        </p>
                                    </div>
                        ...
                        <?php $showtax = $show_invoice_tax == 'TRUE' ? TRUE : FALSE; ?>
                        <div class="line"></div>
                            ...
                            <?php if (User::is_admin() || User::perm_allowed(User::get_id(),'edit_all_invoices')) { ?>
                                ...
                                <?php if ($inv->status != 'Paid') { ?>
                                    <tr class="hidden-print">
                                        ...
                                        <td><input id="auto-quantity" type="text" name="quantity" value="1" class="form-control" style="width: 95px;margin-left: 65px;" ></td>
                                        <?php if ($showtax) : ?>
                                            <td>
                                                <select name="item_tax_rate" class="form-control m-b" style="width: 100px;" >
                                                    ...
                                                    foreach (Invoice::get_tax_rates() as $key => $tax) {
                                                        if($tax->subdomain_id == $this->session->userdata('subdomain_id')){
                                                        ?>
                                                        <option value="<?=$tax->tax_rate_percent?>" <?=$default_tax == $tax->tax_rate_percent ? ' selected="selected"' : '' ?>><?=$tax->tax_rate_name ?></option>
                                                    <?php } } ?>
                                                </select>
                                            </td>
                                        <?php endif; ?>
                                        ...
                                        <?php endif; ?>
                                        <td></td>
                                        <td><button type="submit" class="btn btn-<?=$theme_color;?>"><i class="fa fa-check"></i> <?= lang('save') ?></button></td>
                                        </form>
                                    </tr>
                                <?php } ?>
                                <?php } ?>
                    ...
                    <?php if($inv->allow_stripe == 'Yes') { ?>
                        ...
                        <script>
                            var handler = StripeCheckout.configure({
                                ...
                                image: '<?=base_url()?>assets/images/<?=$site_logo?>',
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <h5><?=lang('do_you_clone_invoice')?>(<?= $inv->reference_no ?>)?</h5>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal"><?=lang('no')?></button>
        <button class="btn btn-primary clone_invoice_btn" data-invoice="<?= $inv->inv_id ?>"><?=lang('yes')?></button>
      </div>
    </div>
  </div>
</div>
