...

{
...

    {
        ...
        $this->template->title(lang('sales').' - '.$this->company_name);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if (config_item('increment_invoice_number') == 'TRUE') {
                    ...
                    $_POST['reference_no'] = $this->invoice_prefix.Invoice::generate_invoice_number();
...

    {
        ...
        if (User::is_admin() || User::perm_allowed(User::get_id(), 'edit_all_invoices')) {
            ...
            $_POST['date_saved'] = Applib::date_formatter($_POST['date_saved']).' 00:00:00';
                ...
                //Get Login User Based on Branch Details
                if($this->session->userdata('role_name')=='admin' && $this->session->userdata('branch_id') != '') {
                    $branch_id = $this->session->userdata('branch_id');
                    $subdomain_id = $this->session->userdata('subdomain_id');
                    $data['branches'] = $this->db->query("SELECT * FROM `dgt_branches` WHERE find_in_set(`id`, '$branch_id') AND `subdomain_id`='".$subdomain_id."' ")->result();
...

    {
        ...
        $new_invoice = array(
            ...
            'reference_no' => $this->invoice_prefix.Invoice::generate_invoice_number(),
