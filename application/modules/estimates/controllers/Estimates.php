...

{
...

    {
        ...
        $this->estimate_prefix = $estimate_settings['estimate_prefix']?$estimate_settings['estimate_prefix']:config_item('estimate_prefix');
        $this->display_estimate_badge = $estimate_settings['display_estimate_badge']?$estimate_settings['display_estimate_badge']:config_item('display_estimate_badge');
        $this->estimate_to_project = $estimate_settings['estimate_to_project']?$estimate_settings['estimate_to_project']:config_item('estimate_to_project');
        ...
        $this->template->title(lang('estimates').' - '.$this->company_name);
...

    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $_POST['reference_no'] = $this->estimate_prefix.Estimate::generate_estimate_number();
            ...
            $data['estimates'] = $this->_estimate_list();
            //Get branch name based on login user
...

    {
        ...
        $pdf = array(
                    ...
                    'badge' => $this->display_estimate_badge,
...

    {
        ...
        if ($status == 'Accepted' && $this->estimate_to_project == 'TRUE') {
...

    {
        ...
        if($this->input->post()) :
            ...
            $ref = $this->invoice_prefix.filter_var($i->reference_no, FILTER_SANITIZE_NUMBER_INT);



            if (config_item('increment_invoice_number') == 'TRUE') {

                $ref = $this->invoice_prefix.Invoice::generate_invoice_number();
