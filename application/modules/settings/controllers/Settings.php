...
class Settings extends MX_Controller {
    ...
    function __construct()
    {
       ...
       // if(!User::is_admin()) redirect('');
        ...
        $this->appsumo_code = '?settings=appsumo_code';
    ...
    public function get_emp_dept()
    {
        ...
        $this->db->where('U.is_teamlead','yes');        
        $emp_details =$this->db->get()->result_array();        
    ...
    //          if ($this->input->post('default_leave_approval') == 'seq-approver') {
    //          $leave_approvers = $this->input->post('leave_approvers');
    //          $leave_approvers_dept = $this->input->post('leave_approvers_dept');
    //         } else{
    //             $leave_approvers = $this->input->post('leave_approvers');
    //             $leave_approvers_dept = $this->input->post('leave_approvers_dept');
    //         }
    ...
    //                     $approvers_details = array(
    //                     'approvers' => $value,
    //                     'dept_id' => $leave_approvers_dept[$key],
    //                     'default_leave_approval' => $this->input->post('default_leave_approval'),
    //                     'created_by'=>$this->session->userdata('user_id'),
    //                     //'lt_incentive_plan' => ($this->input->post('long_term_incentive_plan')?1:0),
    ...
    //                  $args = array(
    //             'user' => $value,
    //             'module' => 'settings/?settings=approval&key=leave_approval',
    //             'module_field_id' => $leave_approver_settings_id,
    //             'activity' => 'Leave approval settings',
    //             'icon' => 'fa-user',
    //             'value1' => $this->input->post('default_leave_approval', true),
    ...
    //         if ($this->input->post('default_expense_approval') == 'seq-approver') {
    //          $expense_approvers = $this->input->post('expense_approvers');
    //          $expense_approvers_dept = $this->input->post('expense_approvers_dept');
    //         } else{
    //             $expense_approvers = $this->input->post('expense_approvers_sim');
    //         }
    //         if (count($expense_approvers) > 0) {
    ...
    //                     $approvers_details = array(
    //                     'approvers' => $value,
    //                     'dept_id' => $expense_approvers_dept[$key],
    //                     'default_expense_approval' => $this->input->post('default_expense_approval'),
    //                     'created_by'=>$this->session->userdata('user_id'),
    //                     //'lt_incentive_plan' => ($this->input->post('long_term_incentive_plan')?1:0),
    ...
    //                  $args = array(
    //             'user' => $value,
    //             'module' => 'settings/?settings=approval&key=expense_approval',
    //             'module_field_id' => $expense_approver_settings_id,
    //             'activity' => 'You are added for Expense approval',
    //             'icon' => 'fa-user',
    //             'value1' => $this->input->post('default_expense_approval', true),
    //         );
    // App::Log($args);
    ...
    public function roles_delete(){
     ...
     public function performance_status(){
        ...
        $args = array(
                    ...
                    'module' => 'performance',
                    'module_field_id' => 1,
                    'activity' => 'Activity updated cperformance status',
                    'icon' => 'fa-user',
    ...
    public function appsumo_code(){        
        if (!empty($_POST['appsumo_code'])) {
            $appsumo_code = $this->input->post('appsumo_code');
            $coupons = $this->db->get_where('coupons',array('appsumo_code'=>$appsumo_code,'status'=>0))->result_array();
            $subscriber_appsumo_code = $this->db->get_where('subscriber_appsumo_code',array('appsumo_code'=>$appsumo_code,'subscriber_id'=>$this->session->userdata('subdomain_id')))->result_array();
            if(count($subscriber_appsumo_code) == 0){ 
                if(count($coupons) > 0){ 
                    
                    $subscriber_appsumo_code = array(
                                'subscriber_id'   => $this->session->userdata('subdomain_id'),
                                'appsumo_code'   => $_POST['appsumo_code'],                            
                                'created'       => date('Y-m-d')
                                
                                );
                        $this->db->insert('dgt_subscriber_appsumo_code',$subscriber_appsumo_code);
                        ...
                        $args = array(
                            'user' => User::get_id(),
                            'module' => 'settings/?settings=appsumo_code',
                            'module_field_id' => 1,
                            'activity' => 'Code applied successfully',
                            'icon' => 'fa-gift'
                        );
                        App::Log($args);
                        $this->session->set_flashdata('tokbox_success', 'Code applied successfully!');

                }else{
                    $this->session->set_flashdata('tokbox_error', 'Code does not exist, please try again.'); 
                }   
                     
            }else{
                 $this->session->set_flashdata('tokbox_error', 'Code already applied!');

            }            
        
        }else{
            $this->session->set_flashdata('tokbox_error', 'Please enter your AppSumo code '); 
        }
        redirect('settings/?settings=appsumo_code');    
    }
    // public function check_appsumo_code()
    // {
    //     $appsumo_code = $this->input->post('appsumo_code');
    //     $res = $this->db->get_where('coupons',array('appsumo_code'=>$appsumo_code,'status'=>0))->result_array();
    //     if(count($res) > 0){ 
    //         $subscriber_appsumo_code = $this->db->get_where('subscriber_appsumo_code',array('appsumo_code'=>$appsumo_code,'subscriber_id'=>$this->session->userdata('subdomain_id')))->result_array();
    //         if(count($subscriber_appsumo_code) == 0){
    //             echo 1; exit(); //success
    //         }else{
    //              echo 2; exit(); //Code already applied
    //         }

    //     }else{
    //         echo 3;// Code does not exist, please try again.
    //     }
