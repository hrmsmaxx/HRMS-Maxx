...
<div class="content">
	...
	<div class="table-responsive">
		<table id="table-expenses" class="table lable-table table-striped custom-table m-b-0">
			<thead>
				<tr>
			...
			<tr id="row_id_<?=$e->id?>" >
				...
				<td class="col-currency" style="text-align: right;">
					<strong>
					<?php
					...
					<?php if($e->currency_symbol != ''){
							$currency = explode('-', $e->currency_symbol);
								echo $currency[0]. ''.$e->amount.' '.$currency[1];
						} else{ 
...
<?php if (($this->tank_auth->user_role($this->tank_auth->get_role_id()) != 'superadmin') && ($this->tank_auth->user_role($this->tank_auth->get_role_id()) != 'admin')) { 
	...
	$check_approver = $this->db->query("SELECT * FROM `dgt_expenses` where FIND_IN_SET(".$this->session->userdata('user_id').", expense_approvers) order by id  ASC ")->num_rows(); 
	if($check_approver > 0){
...
<div class="row" id="team_expense">
	...
	<div class="table-responsive">
		...
		<table id="table-expenses" class="table  lable-table table-striped custom-table m-b-0">
			...
			if($check_teamlead['is_teamlead'] == 'yes'){
			$get_approver_record = $this->db->get_where('dgt_expense_approvers',array('expense'=>$e->id,'approvers'=>$this->session->userdata('user_id'),'view_status'=>1))->row_array();
							  	if(!empty($get_approver_record)){
			?>
			<tr id="row_id_<?=$e->id?>" >
				<td ><?php echo $j;?></td>
				<td>
