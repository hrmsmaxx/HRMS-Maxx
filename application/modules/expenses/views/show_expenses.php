<div class="content">
	<div class="row">
		<div class="col-sm-8 col-xs-3">
			...
			?>
			<h4 class="page-title"><?php echo $username['fullname']?> <?=lang('expenses')?></h4>
		</div>
		<div class="col-sm-4  text-right m-b-20">     
              <a class="btn back-btn" href="<?=base_url()?>expenses"><i class="fa fa-chevron-left"></i> <?=lang('back')?></a>
          </div>
	...
	<?php if (($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'admin') || ($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'superadmin') || ($check_teamlead['is_teamlead'] =='yes')) { 
		...
		<div class="table-responsive">
			...
			<td colspan="6"> <?=lang('no_records_found')?></td>
		<?php } ?>
			</tbody>
		</table>  
	    </div>
