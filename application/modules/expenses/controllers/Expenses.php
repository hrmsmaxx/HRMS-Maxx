...
class Expenses extends MX_Controller {
	...
	function __construct()
	{
		...
		$this->template->title(lang('expenses').' - '.$this->company_name);
...
// 		if(!$this->_can_view_expense($id)){ redirect(); }
	...
	function create()
	{
		...
		if ($this->input->post()) 
		{
			...
			$expense_approvers = $this->db->get_where('expense_approver_settings',array('subdomain_id'=>$this->session->userdata('subdomain_id')))->result_array();
	 	 	...
	 	 	$teamlead_id = $this->db->get_where('users',array('id'=>$this->session->userdata('user_id'), 'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
			
	 	 	if(!empty($approvers_id)){
	 	 		if (in_array(9, $approvers_id))
			  	{
				  	$approvers[] = $teamlead_id['teamlead_id'];
				  	$teamlead_id_details = $this->db->get_where('users',array('id'=>$teamlead_id['teamlead_id'], 'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
    ...
    public function show_expense()
  	{
  		...
  		$this->load->module('layouts');
                ...
                $this->template->title('expenses'.' - '.$this->company_name); 
 				$data['datepicker'] = TRUE;
				$data['form']       = TRUE; 
				$data['datatables'] = TRUE;
                $data['page'] = lang('sales');
