...
// 		if(!$this->_can_view_expense($id)){ redirect(); }
                ...
                if (count($approvers) > 0) {
                    ...
                    foreach ($approvers as $key => $value) {
                        $approvers_details = array(
                        	...
                        	if($expense_approvers[0]['default_expense_approval'] == "seq-approver"){
                        		if($i ==1){
	                    ...
	                    if($expense_approvers[0]['default_expense_approval'] == "seq-approver"){
                    		if($i ==1){    
								...
								'user' => $value,
								'activity' => 'Expense Submitted by '.$approver_det['fullname'],
								'icon' => 'fa-plus',
	...
	{		
		...
		if( (isset($_POST['id'])&&isset($_POST['status'])) && (!empty($_POST['id'])&&!empty($_POST['status'])) )
		{
			...
			// next approvers view
			if($_POST['status'] == 1){				
		        $get_approver_record = $this->db->get_where('dgt_expense_approvers',array('expense'=>$_POST['id'],'approvers'=>$approver))->row_array();	        
				
				$view_next = $this->db->query('select * from dgt_expense_approvers where id = (select min(id) from dgt_expense_approvers where id > '.$get_approver_record['id'].')')->row_array();
				$view_status['view_status'] = 1;
				if(!empty($view_next)){
					$this->db->update('dgt_expense_approvers',$view_status,array('expense'=>$_POST['id'],'id'=>$view_next['id'])); 

					$data = array(
								'module' => 'expenses',
								'module_field_id' => $_POST['id'],
								'user' => $view_next['approvers'],
								'activity' => 'Expense Submitted by '.$acc_det[0]['fullname'],
								'icon' => 'fa-plus',
								// 'value1' => $cur.' '.$this->input->post('amount'),
