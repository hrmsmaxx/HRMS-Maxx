...

{
...

    {
        ...
        if($post['client_name'] != '') {
            $this->db->like('company_name', $post['client_name']);
        }

        if($post['client_email'] != '') {
            $this->db->like('company_email', $post['client_email']);
        }

        $data['companies'] = $this->db->select('*')
                    ->from('companies')
                    ->where(array('is_lead' => 0,'co_id >'=> 0,'subdomain_id'=>$_SESSION['subdomain_id']))
