...
class Promotion extends MX_Controller {
    ...
    function index()
    {
		if($this->tank_auth->is_logged_in()) { 
                ...
                $this->template->title(lang('promotion').' - '.$company_name); 
 				$data['datepicker'] = TRUE;
				$data['form']       = TRUE;
                $data['page']       = lang('all_policies');
                $data['sub_page']       = lang('promotion');
         ...
         foreach ($list as $promotion) {
           ...
           $no++;
            ...
            $row[] = '<div class="user_det_list">
                        <a href="'.base_url().'employees/profile_view/'.$promotion->user_id.'"> <img class="avatar"  src="'.base_url().'assets/avatar/'.$promotion->avatar.'"></a>
                        <h2><span class="username-info">'.$promotion->fullname.'</span>
                        <span class="userrole-info"> '.$promotion->designation.'</span>
                        <span class="username-info">'. $promotion->id_code.'</span></a></h2>
                    </div>';
