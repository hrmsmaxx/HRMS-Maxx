...
class Termination extends MX_Controller {
    ...
    function index()
    {
		if($this->tank_auth->is_logged_in()) { 
                ...
                $this->template->title(lang('termination').' - '.$company_name); 
 				$data['datepicker'] = TRUE;
				$data['form']       = TRUE; 
                $data['page']       = lang('all_policies');
                $data['sub_page']       = lang('termination');
         ...
         foreach ($list as $termination) {
           ...
           $no++;
            ...
            $row[] = '<div class="user_det_list">
                        <a href="'.base_url().'employees/profile_view/'.$termination->user_id.'"> <img class="avatar"  src="'.base_url().'assets/avatar/'.$termination->avatar.'"></a>
                        <h2><span class="username-info">'.$termination->fullname.'</span>
                        <span class="userrole-info"> '.$termination->designation.'</span>
                        <span class="username-info">'. $termination->id_code.'</span></a></h2>
                    </div>';
