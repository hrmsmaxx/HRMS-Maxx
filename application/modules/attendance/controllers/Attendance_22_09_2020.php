   ...
   public function save_punch_details_out(){
            ...
            $month_days_in_out_record = unserialize($record['month_days_in_out']);
             ...
             if(!empty($record_day[$a_day]) && !empty($month_days_in_out_record[$a_day])){
              ...
              if($total_records == 1 && empty($current_day['punch_out'])){
                ...
                $current_days = array('day'=>1,'punch_in'=>$a_cin,'punch_out'=>'','punchin_workcode'=>$workcode,'punch_in_manually_made'=>'yes');
                $month_days_in_out_record[$a_day][0] = $current_days;
              }else{
                
                if(!empty($current_day['punch_in']) && !empty($current_day['punch_out']))
                {
                  $current_days[$total_records] =array('day'=>1,'punch_in'=>$a_cin,'punch_out'=>'','punchin_workcode'=>$workcode,'punch_in_manually_made'=>'yes');
                  $month_days_in_out_record[$a_day] = $current_days;
                } 
              }
    ...
    // echo "<pre>";print_r($_POST); exit;
      ...
      if(!empty($record['month_days'])){
        ...
        if(!empty($record_day[$a_day])){             
          ...
          //notification    
          $page = (($role_id==4) || ($role_id==1))?'attendance':'attendance/team_attendance';
    ...
    function attendance_records()
    {
        if($this->tank_auth->is_logged_in()) {
            ...
            $role_id = $this->tank_auth->get_role_id();
            $page = (($role_id==4) || ($role_id==1) || ($this->tank_auth->user_role($this->tank_auth->get_user_type()) == 'supervisor'))?'attendance_records':'';
            $this->template
                  ->set_layout('users')
                  ->build($page,isset($data) ? $data : NULL);
          }else{
    ...
    function superior_dashboard()
    {
        if($this->tank_auth->is_logged_in()) {
            ...
            $role_id = $this->tank_auth->get_role_id();
            $page = (($this->tank_auth->user_role($this->tank_auth->get_user_type()) == 'supervisor') || ($role_id==1))?'superior_dashboard':'';
            $this->template
                  ->set_layout('users')
                  ->build($page,isset($data) ? $data : NULL);
          }else{
     ...
     function manager_dashboard()
    {
        if($this->tank_auth->is_logged_in()) {
            ...
            $role_id = $this->tank_auth->get_role_id();
            $page = (($this->tank_auth->user_role($this->tank_auth->get_user_type()) == 'manager') || ($role_id==1))?'manager_dashboard':'';
            $this->template
                  ->set_layout('users')
                  ->build($page,isset($data) ? $data : NULL);
          }else{
