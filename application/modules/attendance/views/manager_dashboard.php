                  ...
                  <select class="select2-option form-control" name="user_id" id="user_name">
                        ...
                        <option value=""><?php echo lang('select_employee');?></option> 
                            ...
                            if($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'admin'){  
                              ...
                              $department_id= $this->tank_auth->get_department($userid);
                              ...
                              if($dept_id !=0){   
                               if($this->tank_auth->user_role($c->user_type) != 'manager'){ ?>
                  ...
                  if(isset($_POST['id_code']) && !empty($_POST['id_code'])){
                    ...
                    if(!empty($users)){
                      if($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'admin'){ 
                          ...
                          $manager_dept_id= $this->tank_auth->get_department($this->session->userdata('user_id'));
                          $user_dept_id= $this->tank_auth->get_department($users['id']);
                          if($manager_dept_id == $user_dept_id){
                            ...
                            if(!empty($dept_users)){
                             if($this->tank_auth->user_role($dept_users['user_type']) != 'manager'){ 
               ...
               $dept_id= $this->tank_auth->get_department($this->session->userdata('user_id'));
                if($dept_id !=0){                   
                    ...
                    $dept_users= $this->db->select('*')
                      ...
                      if(!empty($dept_users)){
                        foreach ($dept_users as $key => $value) {
                          if($this->tank_auth->user_role($value['user_type']) != 'manager'){ 
                            $user_id[] = $value['id'];
             ...
             <?php
                    ...
                    if(isset($_POST['range']) && !empty($_POST['range'])){
                            ...
                            if(isset($_POST['range']) && !empty($_POST['range'])){
                              ...
                              // if (date('m', $time)==$month)       
                                ...
                                if(!empty($rows['month_days'])){
                                  ...
                                  if($month_days[$a_day-1]['day_status'] =='closed' && $month_days[$a_day-1]['closed_status'] !='yes'){
                     ...
                     <tr>
                      ...
                      <td><form  action="<?php echo base_url(); ?>attendance/update_day_closed" method="POST" class="form-horizontal">
                        <input type="hidden" name="user_id" value="<?php echo $user_id?>">
                        <input type="hidden" name="day" value="<?php echo $a_day?>">
                        <input type="hidden" name="month" value="<?php echo $a_month?>">
                        <input type="hidden" name="year" value="<?php echo $a_year?>">
                       
                       <div class="row">
                          <div class="col-md-12">
                            <!-- <a href="#" type="submit" class="btn btn-primary pull-right m-l-10"><?php echo lang('save');?></a> -->
                            <button type="submit" class="btn btn-primary" data-toggle="tooltip" title="<?=lang('closed')?>" ><?=lang('closed')?></button>
