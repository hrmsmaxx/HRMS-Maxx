...
class Budgets extends MX_Controller {
...

	{
		...
		$this->template->title(lang('budgets').' - '.$this->company_name);

		$data['page'] = lang('accounting');

		$data['sub_page'] = lang('budgets');

		$data['datatables'] = TRUE;

		$data['form'] = TRUE;

		$data['budgets'] = $this->db->get_where('budgets',array('subdomain_id'=>$this->session->userdata('subdomain_id')))->result_array();
