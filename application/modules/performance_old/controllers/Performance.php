...
class Performance extends MX_Controller {
     ...
     public function performance_dashboard(){
        ...
        if($data['role'] == '1') {
             ...
             $data['performances_360'] = $this->db->select('IFNULL(ROUND(AVG(self_rating),1),0) as self_ratings, IFNULL(ROUND(AVG(rating),1),0) as your_ratings,user_id,IFNULL(ROUND(AVG(self_rating+rating),1),0) as peer_ratings')
               ...
               $data['completed_performance'] = $this->db->where('status',1)->where('subdomain_id',$this->session->userdata('subdomain_id'))->from('performance_360')->get()->num_rows();

               $data['outstanding_performance'] = $this->db->where('status',0)->where('subdomain_id',$this->session->userdata('subdomain_id'))->from('performance_360')->get()->num_rows();

               } elseif($data['role'] == '3'){



                     $data['performances_360'] = $this->db->select('IFNULL(ROUND(AVG(self_rating),1),0) as self_ratings, IFNULL(ROUND(AVG(rating),1),0) as your_ratings,user_id,IFNULL(ROUND(AVG(self_rating+rating),1),0) as peer_ratings')->where('teamlead_id',$user_id)->where('subdomain_id',$this->session->userdata('subdomain_id'))->group_by('user_id')->order_by('id','ASC')->get('performance_360')->result_array(); 
    ...
    { 
        ...
        if($this->tank_auth->is_logged_in()) { 
                ...
                $this->template->title(lang('performance')); 
     ...
     {      
      ...
      // }else{
        ...
        $this->session->set_flashdata('tokbox_success', lang('performance_added_successfully'));
...

     {
        ...
        //     $this->db->update('dgt_okrdetails',$data);
         ...
         $this->session->set_flashdata('tokbox_success', lang('added_uccessfully'));
