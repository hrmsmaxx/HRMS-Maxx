...
class Holidays extends MX_Controller {
...

    {
		...
		if($this->tank_auth->is_logged_in()) {
			...
			$this->template->title(lang('holidays').'-'.$this->company_name);
			...
			$data['sub_page']           = lang('holidays');

			$data['role']           = $this->tank_auth->get_role_id();

			$data['holidays']       = $this->db->query("select * from dgt_holidays where DATE_FORMAT(holiday_date,'%Y') = ".date('Y')." and status = 0 and subdomain_id = ".$this->session->userdata('subdomain_id')." order by holiday_date  ASC ")->result_array();
...

	{
		 ...
		 $holidays = $this->db->query("select * from dgt_holidays where DATE_FORMAT(holiday_date,'%Y') = ".$year." and status = 0 and subdomain_id = ".$this->session->userdata('subdomain_id')." order by holiday_date ASC ")->result_array();
