...
<div class="card-box m-b-0">
	<div class="row">
		...
		<div class="col-xs-4">
			<h4 class="page-title"><?php echo lang('employees');?></h4>

		</div>
		<div class="col-sm-8 col-9 text-right m-b-20">

			<?php $subscribers_data =  $this->db->where('subscribers_id',$this->session->userdata('subdomain_id'))->get('dgt_subscribers')->row();
				 $this->db->select('count(*) as total_users');
				 $this->db->where('subdomain_id',$this->session->userdata('subdomain_id'));
				 $this->db->group_by('subdomain_id');
				 $users_count = $this->db->get('users')->row();
				 ;
				 //echo $subscribers_data->user_count;
				 if(($users_count->total_users < $subscribers_data->user_count) || empty($subscribers_data)){
			?>
			<a href="javascript:void(0)" class="btn add-btn" data-toggle="modal" data-target="#add_new_user"><i class="fa fa-plus"></i><?php echo lang('add_employee');?></a>
		<?php }else{
			echo '<a href="javascript:void(0)" class="btn btn-warning">You have reached your maximum number of employees. Please contact support for more info</a>';

		}?>

			<div class="view-icons">
				<a href="<?php echo base_url(); ?>employees" class="list-view btn btn-link"><i class="fa fa-bars"></i></a>
				<a href="<?php echo base_url(); ?>employees/grid_employees" class="grid-view btn btn-link active"><i class="fa fa-th"></i></a>
			</div>
		</div>
	</div>	
 ...
 <?php if ($this->tank_auth->user_role($this->tank_auth->get_role_id()) == 'admin') { ?>
	...
	<?php } ?>
		<div class="col-sm-6 col-xs-6 col-md-3">  
			<a href="javascript:void(0)" id="employee_search_btn" onclick="filter_next_page(1)" class="btn btn-success btn-block btn-searchEmployee btn-circle m-b-10"> <?php echo lang('search');?> </a>  
		</div>  
	</div>
	
	<div class="row">
		<div class="col-md-12">
			<div id="employees_details" data-view="grid"></div>
		</div>
	</div>
</div>
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<select class="select2-option" multiple="multiple" name="branch_id[]" id="branch_id">
						...
						<div class="col-sm-6">
							<div class="form-group">
								...
								<select class="select2-option" multiple="multiple" style="width:100%;" name="department_name[]" id="department_name">
					...
					<div class="row">
						...
						<div class="col-sm-6">  
							<div class="form-group">
								...
								<input class="form-control" type="text" value="" name="emp_doj" id="emp_doj" data-date-format="yyyy-mm-dd" >
							</div>
						</div>
					</div>
