...
$inv = Subscriber_invoice::view_by_id($id);
$l = Subscriber_client::view_by_id($inv->client)->language;
...
<body>
<?php
$watermark = $lang2[Subscriber_invoice::payment_status($inv->inv_id)];
...
<div style="margin-bottom: 20px; margin-top: 30px;">
    <table width="100%" cellpadding="10" style="vertical-align: top;">
        ...
        <?=Subscriber_client::view_by_id($inv->client)->company_name;?></span><br/>
                ...
                <?php if(Subscriber_client::view_by_id($inv->client)->zip != '') {
                    echo ", ".Subscriber_client::view_by_id($inv->client)->zip;
                } ?><br/>
                <?php if (Subscriber_client::view_by_id($inv->client)->state != '') {
                    echo Subscriber_client::view_by_id($inv->client)->state.", ";
                } ?>
                <?=Subscriber_client::view_by_id($inv->client)->country; ?> <br/>
                <?php $phone = Subscriber_client::view_by_id($inv->client)->company_phone; ?>
                <?php if ($phone != '') : ?>
                    <span><?= $lang2['phone'] ?> | </span><?= $phone ?><br/>
                <?php endif; ?>
                <?php $fax = Subscriber_client::view_by_id($inv->client)->company_fax; ?>
                <?php if ($fax != '') : ?>
                    <span><?= $lang2['fax'] ?> | </span><?= $fax ?><br/>
                <?php endif; ?>
                <?php $vat = Subscriber_client::view_by_id($inv->client)->VAT; ?>
                <?php if ($vat != '') : ?>
...
<table class="items" width="100%" style="border-spacing:3px; font-size: 9pt; border-collapse: collapse;" cellpadding="10">
    ...
    <tbody>
    <!-- ITEMS HERE -->
    <?php foreach (Subscriber_invoice::has_items($id) as $idx => $item) { ?>
        <tr<?= $idx + 1 == count(Subscriber_invoice::has_items($id)) ? ' class="last"' : ''?>>
            <?php if($show_invoice_tax == 'FALSE') : ?>
    ...
    <tr class="first">
        ...
        <td style="font-weight: bold; color: #111111; text-align: right;"><?=Applib::format_currency($inv->currency, Subscriber_invoice::get_invoice_subtotal($inv->inv_id)) ?></td>
    </tr>
    <?php if ($inv->tax > 0): ?>
        <tr>
            <td colspan="<?=$colspan?>" style="background-color:#ffffff;"></td>
            ...
            <td style="font-weight: bold; color: #111111; text-align: right;"><?=Applib::format_currency($inv->currency,Subscriber_invoice::get_invoice_tax($inv->inv_id)) ?></td>
        </tr>
    <?php endif ?>
    ...
    <?php if ($inv->tax2 > 0): ?>
        <tr>
            ...
            <td style="font-weight: bold; color: #111111; text-align: right;"><?=Applib::format_currency($inv->currency,Subscriber_invoice::get_invoice_tax($inv->inv_id,'tax2')) ?></td>
        </tr>
    <?php endif ?>
    ...
    <?php if ($inv->discount > 0) { ?>
        <tr>
            ...
            <td style="font-weight: bold; color: #111111; text-align: right;"><?=Applib::format_currency($inv->currency,Subscriber_invoice::get_invoice_discount($inv->inv_id)) ?></td>
        </tr>
    <?php } ?>
    <?php if ($inv->extra_fee > 0) { ?>
        <tr>
            ...
            <td style="font-weight: bold; color: #111111; text-align: right;"><?=Applib::format_currency($inv->currency,Subscriber_invoice::get_invoice_fee($inv->inv_id)) ?></td>
        </tr>
    <?php } ?>
    <?php
    $payment_made = Subscriber_invoice::get_invoice_paid($inv->inv_id);
    if ($payment_made > 0) { ?>
        <tr>
            <td colspan="<?=$colspan?>" style="background-color:#ffffff;"></td>
    ...
    <tr>
        ...
        <td style="font-weight: bold; color: #111111; text-align: right; background-color: <?=$color?>; color:#ffffff;"><?=Applib::format_currency($inv->currency, Subscriber_invoice::get_invoice_due_amount($inv->inv_id)) ?></td>
    </tr>
