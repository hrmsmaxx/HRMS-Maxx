...
<?php if($this->session->userdata('user_id') !=2){ ?>
	...
	<div class="row">
		<div class="col-md-8">
			<h4 class="page-title"><?=lang('invoice_payments')?></h4>
		</div>
...
<div class="card-box hidden-print">
	<div class="row">
		<div class="col-sm-8">
			<a href="<?=site_url()?>subscriber_invoices/view/<?=$inv->inv_id?>" class="btn btn-sm btn-success">
				<?=lang('view_invoice')?>
			</a>
			<?php if(User::is_admin() || User::perm_allowed(User::get_id(),'edit_all_invoices')) { ?>
				<?php if ($inv->show_client == 'Yes') { ?>
					<a class="btn btn-sm btn-danger" href="<?= base_url() ?>subscriber_invoices/hide/<?= $inv->inv_id ?>" data-toggle="tooltip" data-title="<?= lang('hide_to_client') ?>" data-placement="bottom"><i class="fa fa-eye-slash"></i>
					</a>

				<?php } else { ?>

					<a class="btn btn-sm btn-success" href="<?= base_url() ?>subscriber_invoices/show/<?= $inv->inv_id ?>" data-toggle="tooltip" data-title="<?= lang('show_to_client') ?>" data-placement="bottom"><i class="fa fa-eye"></i>
					</a>
				<?php } ?>
			<?php } ?>
		...
		<div class="col-sm-3 pull-right">
			...
			<?php elseif (config_item('pdf_engine') == 'mpdf') : ?>
				<a href="<?= base_url() ?>subscriber_invoices/pdf/<?=$inv->inv_id ?>" class="btn btn-sm btn-primary pull-right"><i class="fa fa-file-pdf-o"></i> <?=lang('pdf') ?></a>
			<?php endif; ?>
		</div>
	</div>
</div>
                                    ...
                                    <table id="table-payments" class="table table-striped custom-table m-b-0 AppendDataTables">
                                        ...
                                        <?php foreach ($payments as $key => $p) { ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php echo Subscriber_client::view_by_id($p->paid_by)->company_name; ?>
                                                </td>
