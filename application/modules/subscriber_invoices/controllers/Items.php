...
class Items extends MX_Controller {
	...
	function __construct()
	{
		...
		$this->load->model(array('Subscriber_invoice','App','Client'));
	...
	function add(){
		...
		{	
			...
			if ($tax_rate == '0.00') {
				if($row = $this->db->where('item_name',$item_name)->get('subscriber_items_saved')->row()){
			...
			// unset($_POST['tax']);
				...
				if(App::save_data('subscriber_items',$data)){
	...
	function edit(){
		...
		$invoice_id = Subscriber_invoice::view_item($item_id)->invoice_id;
		...
		{	
			...
			if(App::update('subscriber_items', array('item_id' => $item_id),$data)){
					// Applib::go_to('invoices/view/'.$invoice_id,'success',lang('item_added_successfully'));
					$this->session->set_flashdata('tokbox_success', lang('item_added_successfully'));
        			redirect('subscriber_invoices/view/'.$invoice_id);
			}
	...
	function insert()
	{
		if ($this->input->post()) {
            ...
            $items = Subscriber_invoice::has_items($invoice);
			...
			if(App::save_data('subscriber_items',$form_data)){
	...
	function delete(){
		if ($this->input->post() ){
					...
					$invoice = $this->input->post('invoice', TRUE);
				if(App::delete('subscriber_items',array('item_id' => $item_id))){
	...
	function reorder(){
                if ($this->input->post() ){
                        ...
                        foreach ($items[0] as $ix => $item) {
                            App::update('subscriber_items', array('item_id' => $item->id),array("item_order"=>$ix+1));
                        }
                }
                $data['json'] = $items;
