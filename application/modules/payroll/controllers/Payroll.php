...
class Payroll extends MX_Controller {
    ...
    function index()
    {
		...
		$this->template->title(lang('payroll').' - '.$this->company_name);
		 
		$data['datepicker'] = TRUE;
		$data['form']       = TRUE; 
		$data['page']       = lang('payroll');
		$data['sub_page']       = lang('payrun');
	...
	{ 
 	...
 	function payroll_items()
 	{
 		$this->load->module('layouts');
		$this->load->library('template');
		$this->template->title(lang('payroll_items').' - '.$this->company_name);
		$data['datepicker'] = TRUE;
		$data['form']       = TRUE; 	
		$data['page']       = lang('payroll');
		$data['sub_page']       = lang('payroll_items');
		$data['role']       = $this->tank_auth->get_role_id();
 	...
 	function overtime()
 	{
 		$this->load->module('layouts');
		$this->load->library('template');
		$this->template->title(lang('overtime').' - '.$this->company_name);
		$data['datepicker'] = TRUE;
		$data['form']       = TRUE; 
		$data['page']       = lang('payroll');
		$data['sub_page']       = lang('overtime');
		$data['role']       = $this->tank_auth->get_role_id();
 	...
 	function settings()
	{
        ...
        redirect('payroll/settings');
			...
			$this->load->module('layouts');
		$this->load->library('template');
		$this->template->title(lang('payrollsettings').' - '.$this->company_name);
		$data['datepicker'] = TRUE;
		$data['form']       = TRUE; 
		$data['page']       = lang('payroll');
		$data['sub_page']       = lang('payrollsettings');
		$data['role']       = $this->tank_auth->get_role_id();
