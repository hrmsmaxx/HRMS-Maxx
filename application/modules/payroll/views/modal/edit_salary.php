<div class="modal-dialog modal-lg">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h4 class="modal-title"><?php echo lang('edit_salary'); ?></h4>
		</div> 
		<div class="modal-body">
			<?php $attributes = array('class' => 'form-horizontal','id'=>'payrollSalaryEdit'); echo form_open(base_url().'payroll/save_salary',$attributes); ?>
				<div class="form-group">
					<label class="col-md-3 control-label"> <?php echo lang('new_salary_amount'); ?> <span class="text-danger">*</span></label>
					<div class="col-md-6"> 
					...
					<div class="col-md-3"> 
						<button type="submit" class="btn btn-success btn-md" id="payroll_salary_edit"> <?php echo lang('save_new_salary'); ?> </button>
					</div>
				</div> 
			</form>
			<hr>
			<h4 class="m-b-15"> <?php echo lang('previous_salary_amount'); ?> </h4>
			<?php
			...
			<div class="row m-b-15">
				...
				<div class="col-md-3 slry_sw_<?=$slry['id']?>" style="display:none"> 
					<a href="javascript:;" class="btn btn-success" onClick="staff_salary_update(<?=$slry['id']?>);"> <?php echo lang('update'); ?> </a>
					<a href="javascript:;" class="btn btn-danger" onClick="$('.slry_sw_<?=$slry['id']?>').hide();$('.slry_cls_<?=$slry['id']?>').show();"> <?php echo lang('close'); ?> </a>
				</div>
				<div class="col-md-3 slry_cls_<?=$slry['id']?>"> 
					<?php echo $slry['amount']; ?> &nbsp; &nbsp; <a href="javascript:;" class="btn btn-warning" onClick="$('.slry_cls_<?=$slry['id']?>').hide();$('.slry_sw_<?=$slry['id']?>').show();$('.user_salary_amnt_<?=$slry['id']?>').focus();" > <?php echo lang('edit'); ?> </a> 
				</div>
			</div> 
			<?php }
			}else{?>
				<p style="color:red"> <?php echo lang('no_results_found'); ?> </p>
			<?php } ?> 
  		</div>
