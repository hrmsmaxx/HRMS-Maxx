...
class Shift_scheduling extends MX_Controller {
    ...
    if($_POST){
     // echo "<pre>"; print_r($_POST); exit()   ;
        ...
        // echo $diff; exit;
        $this->db->select('S.*,D.designation,A.fullname');
   ...
   // echo "<pre>";print_r($data['shifts']); exit();
    ...
    function get_shift_by_id(){
        ...
        if($this->input->post()){
            ...
            $records['end_date'] = $record['end_date'];
                  ...
                  if (count($employees) > 0) {
                    ...
                    foreach ($employees as $key => $value) {
                        ...
                        if(isset($_POST['single_insert'])){
                            ...
                            $employee_shifts = $this->db->get_where('shift_scheduling',array('employee_id'=>$value,'schedule_date'=>$_POST['schedule_date']))->row_array();
                             // echo $employee_shifts; exit;
                               if(empty($employee_shifts)){
                                    ...
                                    'color' => $_POST['color'],
                                    'accept_extras' => $_POST['accept_extras'],
                                    'recurring_shift' => isset($_POST['recurring_shift'])?$_POST['recurring_shift']:0,
                                    'repeat_week' => isset($_POST['repeat_week'])?$_POST['repeat_week']:0,
                                    'week_days' => $week_days,
                          ...
                          if(!empty($_POST['end_date'])){
                            ...
                            $end_schedulde_date= date('Y-m-d',strtotime('+60 day'));
                          }
                        ...
                        for ($i=$start_date; $i<=$maxDays ; $i++) { 
                            $your_date = strtotime('+'.$j.' days', strtotime($_POST['schedule_date']));
                            $new_date = date("Y-m-d", $your_date);
                            $employee_shifts = $this->db->get_where('shift_scheduling',array('employee_id'=>$value,'schedule_date'=>$new_date))->row_array();
                             // echo $employee_shifts; exit;
                             // echo "<pre>";
                               if(empty($employee_shifts)){
                             $shift_details = array(
    ...
    function edit_schedule()
    {
         ...
         if($_POST){
            $employees = $_POST['employee'];
           App::delete('shift_scheduling',array('employee_id'=>$employees[0], 'schedule_date >=' => date('Y-m-d',strtotime($_POST['schedule_date']))));
           // echo $this->db->last_query(); exit;
               // echo "<pre>"; print_r($_POST); exit();
                  ...
                  if (count($employees) > 0) {
                    ...
                    foreach ($employees as $key => $value) {
                        ...
                        if(isset($_POST['single_insert'])){
                            if( $_POST['schedule_date'] != $_POST['end_date']){
                                if(!empty($_POST['end_date'])){
                                    $end_schedulde_date= $_POST['end_date'];
                                    $end_schedulde_date= date('Y-m-d',strtotime('+1 day', strtotime($_POST['end_date'])));
                                } 
                         ...
                         // echo "<pre>";print_r($shift_details); exit();
                         $this->db->where('id',$_POST['id']);
                        $this->db->update('shift_scheduling',$shift_details);
                        $shift_id =$this->db->insert_id();
                            }
