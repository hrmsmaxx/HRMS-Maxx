...
class Updates extends MX_Controller {
    ...
    function index() {
        ...
        $this->template->title(lang('updates') . ' - ' .$this->company_name);
    ...
    function db_upd_users() {
        ...
        $language = $this->default_currency;
...
//        $this->db_upd_tables($src['data']);
    ...
    function mysql_backup() {
        ...
        if (!write_file('./assets/backup/database-full-backup_' . date('Y-m-d') . '.zip', $backup)) {
            ...
            $this->session->set_flashdata('tokbox_error', 'Database backup failed cannot write to /assets/database.backup folder.');
            redirect('updates');
        }
