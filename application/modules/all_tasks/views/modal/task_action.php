		...
		<?php $attributes = array('class' => 'bs-example form-horizontal','id' => 'projectAddTask');
          ...
          <input type="hidden" name="project" value="<?=$project?>">
				...
				<div class="col-lg-8">
					...
					<input id="auto-task-name" type="text" class="typeahead form-control" placeholder="<?=lang('task_name');?>" name="task_name_auto">
				</div>
				</div>
            <?php /*if(!User::is_client()){ ?>
				...
				<div class="col-lg-8">
				<select name="milestone" id="mile_stone_name" class="form-control" onchange="check_milestone_date()">
...
<?php } ?>
            ...
            <?php if(!User::is_client()){ ?>
				...
				<option value="<?=$user->assigned_user?>"><?php echo User::displayName($user->assigned_user);?></option>
					<?php } ?>
				</select>
				</div>
				</div>
...
<?php } ?>
		...
		<div class="submit-section">
			<button type="submit" class="btn btn-primary submit-btn" id="project_add_task"><?=lang('submit');?></button>
		</div>
		</form>
		</div>
	</div>
...
.datepicker{z-index:1151 !important;}
		...
		<?php $attributes = array('class' => 'bs-example form-horizontal','id'=>'projectEditTask');
...
<?php } ?>
		...
		<button type="submit" class="btn btn-success" id="project_edit_task"><?=lang('save_changes')?></button>
		</form>
		</div>
	</div>
	<!-- /.modal-content -->
    ...
    //autoclose: true
        ...
        if(milestone != 0 )
        {
            $.post(base_url+'projects/checkmilestone_date/',{id:milestone},function(res){
                        ...
                        $('#edit_task_date_start').datepicker('setStartDate', result.start_date);
                        $('#edit_task_date_start').datepicker('setEndDate', result.due_date);
                        $('#edit_task_date_due').datepicker('setStartDate', result.start_date);
                        $('#edit_task_date_due').datepicker('setEndDate', result.due_date);
                    });
        }else{
            var minDate = new Date(selected.date.valueOf());
...
<?php $attributes = array('class' => 'bs-example form-horizontal','id' => 'projectAddTemplateForm');
		...
		<div class="modal-footer"> <a href="#" class="btn btn-danger" data-dismiss="modal"><?=lang('close')?></a>
		<button type="submit" class="btn btn-success" id="project_add_template"><?=lang('add_task')?></button>
		</form>
		</div>
	</div>
	<!-- /.modal-content -->
...
<div class="modal-dialog">
	<div class="modal-content">
		...
		<button type="button" class="close" data-dismiss="modal">&times;</button> <h4 class="modal-title"><?=lang('upload_file')?></h4>
					...
					<?php
			 $attributes = array('class' => 'bs-example form-horizontal','id'=>'projectTaskFiles');
		...
		<button id="project_task_files" class="btn btn-success"><?=lang('upload_file')?></button>
		</form>
		</div>
	</div>
    ...
    <script type="text/javascript">
        $('#clear-files').click(function(){
            $('#file_container').html(
                "<div class='form-group'>" +
                    "<div class='col-lg-offset-3 col-lg-9'>" +
                    "<label class='btn btn-default btn-choose'>Choose File</label><input class='form-control' type='file' name='taskfiles[]'>" +
                    "</div></div>"
            );
        });
...
<div class="modal-dialog">
	<div class="modal-content">
		...
		<?php $attributes = array('class' => 'bs-example form-horizontal','id'=>'projectTaskFilesU');
		...
		<div class="modal-body">
                ...
                <div class="modal-footer">
                    <a href="#" class="btn btn-danger" data-dismiss="modal"><?=lang('close')?></a>
                    <button id="project_task_filesU" class="btn btn-success"><?=lang('save_file')?></button>
		</form>
		</div>
	        </div>
        </div>
