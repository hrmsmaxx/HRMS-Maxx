                      ...
                      $all_users =  $this->db->get('attendance_details')->result_array();*/
                       ...
                       if($branch_id != '') {
                        ...
                        $all_users = $this->db->select('ad.*')
                                    ->from('attendance_details ad')
                                    ->join('users u', 'u.id=ad.user_id')
                                    ->where(array('a_month >='=>$start_month, 'a_month <='=>$end_month, 'a_year >='=>$start_year,'a_year <='=>$end_year, 'ad.subdomain_id'=> $subdomain_id,'u.id!='=>$this->session->userdata('user_id')))
                                    ->get()
                                    ->result_array();
                    ...
                    if(isset($_POST['range']) && !empty($_POST['range'])){
                      ...
                      if($branch_id != '') {
                        ...
                        $results = $this->db->select('month_days,month_days_in_out')
                                    ->from('attendance_details ad')
                                    ->join('users u', 'u.id=ad.user_id')
                                    ->where(array('user_id'=>$user_id, 'a_month '=>$start_month, 'a_year '=>$start_year,'ad.subdomain_id'=> $subdomain_id,'u.id!='=>$this->session->userdata('user_id')))
                                    ->get()
                                    ->result_array();
