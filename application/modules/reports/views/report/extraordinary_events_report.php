...
<div class="content">
	<div class="panel panel-white">
		<div class="panel-heading">
			<div class="row">
				<div class="col-sm-8">
					<h4 class="page-title m-b-0"><?=lang('extraordinary_events_report')?></h4>
				</div>
			            ...
			            <ul class="dropdown-menu export" style="left:auto; right:0px !important; min-width: 93px !important">  
			              <li>
			                <form method="post" action="">
			                    ...
			                    <input type="hidden" class="form-control work_code_time_excel" name = "work_code_time" value="<?php echo (isset($_POST['work_code_time']) && !empty($_POST['work_code_time']))?$_POST['work_code_time']:'';?>">
			              ...
			              <li>
			                <?php  $report_name = lang('extraordinary_events_report');?>
			                 <button class="btn  btn-block" onclick="extraordinary_events_excel('<?php echo $report_name;?>','excel_export_id');" style="text-align: left;"> <span style="font-size: 18px;text-align: left;"><i class="fa fa-file-excel-o" aria-hidden="true"></i></span><span><?=lang('excel')?></span> </button>
			              </li>
			            </ul>
		          	</div>
		        ...
		        <div class="row">
		          ...
		          <div class="col-md-3">
		            <div class="form-group">
		      		...
		      		<div class="col-md-3">
						<div class="form-group">
							<label><?=lang('work_code_time')?></label>
							<select class="select form-control" name="work_code_time">
								<option selected="selected">-</option>
								<option value="yes" <?php echo (isset($_POST['work_code_time']) && ($_POST['work_code_time'] == 'yes'))?"selected":""?>><?php echo lang('yes'); ?></option>
								<option value="no" <?php echo (isset($_POST['work_code_time']) && ($_POST['work_code_time'] == 'no'))?"selected":""?>><?php echo lang('no'); ?></option>
							<select>
						</div>
					</div>
		          <div class="col-md-2">  
				...
				{                 	  
                  ...
                  if(isset($_POST['department_id']) && !empty($_POST['department_id'])){
                    ...
                    $dept_users= $this->db->select('*')
                    			...
                    			->where("department_id IN (".$dept_id.")",NULL, false)
		                ...
		                for ($m=$month_start; $m <=$month_end ; $m++) {
		                  ...
		                  //echo "<pre>"; print_r($rows); exit;
	                   		if(!empty($rows['month_days'])){
	                   			...
	                   			//Get Multiple Dept Name
	                   			$deptid  = explode(',', $user_details['department_id']);
		                        $department = array();
			                  ...
			                  <!-- </td>			                   -->
			                  <td><?php echo implode(',', $day_lists);?></td>
			                  <td><?php echo !empty(array_sum($missing_works))?intdiv(array_sum($missing_works), 60).'.'. (array_sum($missing_works) % 60).' hrs':'0';?></td>
			                  <td><?php echo implode(',', $workcode);?></td>
			                  <td><?php echo implode(',', $work_code_times);?></td>
			                  <td><?php echo implode(',', $all_comments);?></td>
							</tr>
			              	<?php }
