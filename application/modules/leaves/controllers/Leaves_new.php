...
class Leaves extends MX_Controller {
		...
		$user_id = $this->session->userdata('user_id');
		 ...
		 if ($this->input->post()) {
	 	 	...
	 	 	foreach ($leave_approvers as $key => $value) {
	 	 		if($user_id != $value['approvers']){
	 	 			$approvers_id[] = $value['approvers'];
	 	 			$default_leave_approval = $value['default_leave_approval'];
	 	 		}
	...
	function approve()
	{
		...
		if ($this->input->post()) {
			...
			$approvers_status['view_status'] = 1;
	        ...
	        // $this->db->update('dgt_user_leaves',$det,array('id'=>$this->input->post('req_leave_tbl_id'))); 
	        // next approvers view
	        $get_approver_record = $this->db->get_where('dgt_leave_approvers',array('leave_id'=>$this->input->post('req_leave_tbl_id'),'approvers'=>$approver))->row_array();	        
			...
			if(!empty($view_next)){
				$this->db->update('dgt_leave_approvers',$view_status,array('leave_id'=>$this->input->post('req_leave_tbl_id'),'id'=>$view_next['id'])); 
			}
			

			// End next approvers view
			...
			$leave_approvers = $this->db->get('leave_approver_settings')->result_array();
	 	 	...
	 	 	foreach ($leave_approvers as $key => $value) {
	 	 		if($leave_det[0]['user_id'] != $value['approvers']){
		 	 		$approvers_id[] = $value['approvers'];
		 	 		 $args = array(
	                            'user' => $value['approvers'],
	                            'module' => 'Leaves',
	                            'module_field_id' => $leave_tbl_id,
	...
	function cancel()
	{
		...
		if ($this->input->post()) {
			...
			$leave_approvers = $this->db->get('leave_approver_settings')->result_array();
			foreach ($leave_approvers as $key => $value) {
				if($approver != $value['approvers']){
	 	 		$approvers_id[] = $value['approvers'];
	 	 		$user_details = $this->db->get_where('users',array('id'=>$value['approvers']))->row_array();
					if(!empty($user_details)){
						$approvers[] = $value;	
						$recipient[] = $user_details['email'];
					}
	 	 		$data = array(
						...
						'module_field_id' => $this->input->post('req_leave_tbl_id'),
						'user' => $value['approvers'],
						'activity' => 'Leave Cancelled by '.$approver_det['fullname'],
						'icon' => 'fa-plus',
						'value1' => $cur.' '.$this->input->post('reason'),
						'value2' => $approver
						);
				...
				$message         = '<div style="height: 7px; background-color: #535353;"></div>
										<div style="background-color:#E8E8E8; margin:0px; padding:55px 20px 40px 20px; font-family:Open Sans, Helvetica, sans-serif; font-size:12px; color:#535353;">
											<div style="text-align:center; font-size:24px; font-weight:bold; color:#535353;">Leave Request Cancelled</div>
											<div style="border-radius: 5px 5px 5px 5px; padding:20px; margin-top:45px; background-color:#FFFFFF; font-family:Open Sans, Helvetica, sans-serif; font-size:13px;">
												<p> Hi ,</p>
												<p> Your Leave Request for '.date('d-m-Y',strtotime($leave_det[0]['leave_from'])).' to '.date('d-m-Y',strtotime($leave_det[0]['leave_to'])).' has been Cancelled by '.$approver_det['fullname'].' </p> 
												<br>  
												<a style="text-decoration:none;" href="'.base_url().'leaves/"> 
													<button style="background: #CCCC00; border-radius: 5px;; cursor:pointer"> Click to Login </button> 
												</a>
