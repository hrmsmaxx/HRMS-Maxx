...
class Leaves extends MX_Controller {
...

    {
		...
		if($this->tank_auth->is_logged_in()) { 
                ...
                $data['leave_list1'] = $this->db->select('ul.*,lt.leave_type as l_type,ad.fullname')
                		->from('user_leaves ul')
                		->join('dgt_common_leave_types lt','lt.leave_id = ul.leave_type','LEFT')
                		->join('dgt_account_details ad','ad.user_id = ul.user_id','LEFT')
                		->join('dgt_users u','u.id = ul.user_id','LEFT')
                		->distinct()
                		//->where('DATE_FORMAT(ul.leave_from,%Y)', date('Y'))
                		 ->where("DATE_FORMAT(ul.leave_from,'%Y')", date('Y'))
                		 ->where('ul.subdomain_id', $this->session->userdata('subdomain_id'))
                		->order_by('ul.id', 'DESC')
                		->get()
                		->result_array();
                //echo 'leatc<pre>'; print_r($data['leave_list1']); exit;
                /*if($_POST){
                	...
                	$this->db->join('dgt_account_details ad','ad.user_id = ul.user_id','LEFT');

                	$this->db->get('user_leaves ul')->result_array();
...

	{
		 ...
		 if ($this->input->post()) {
	 	 	...
	 	 	$leave_approvers = $this->db->get_where('leave_approver_settings',array('subdomain_id'=>$this->session->userdata('subdomain_id')))->result_array();
	 	 	...
	 	 	if(!empty($approvers_id)){
...

			  	{
				  	...
				  	$teamlead_id_details = $this->db->get_where('users',array('id'=>$this->input->post('teamlead_id'),'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
				  	...
				  	if (($key = array_search(9, $approvers_id)) !== false) {
						...
						if(!empty($approvers_id)){
							...
							foreach ($approvers_id as $key => $value) {
								...
								$user_details = $this->db->get_where('users',array('designation_id'=>$value,'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
	  	...
	  	// 	echo "<pre>"; print_r($approvers); 
			 ...
			 // echo "<pre>"; print_r($_POST); exit;
	 	 	...
	 	 	$leave_approvers = $this->db->get_where('designation',array('subdomain_id'=>$this->session->userdata('subdomain_id')))->result_array();
			...
			for($i = $d1; $i <= $d2; $i += 86400 ){  $array1[] = $i; }  
 			...
 			if($contdtn){
				...
				$det['subdomain_id']  = $this->session->userdata('subdomain_id');
				...
				$log_detail = $this->db->get_where('dgt_users',array('id'=>$user_id,'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
				...
				// {
					...
					$send_mail = $this->db->get_where('dgt_users',array('id'=>$log_detail['teamlead_id'],'subdomain_id'=>$this->session->userdata('subdomain_id')))->row_array();
	...
	{ 
		...
		$qry    =  "SELECT DISTINCT ul.*,lt.leave_type as l_type,ad.fullname

					FROM `dgt_user_leaves` ul

					left join dgt_leave_types lt on lt.leave_type = ul.leave_type
...

  	{
  		...
  		$this->load->module('layouts');
                ...
                $this->template->title('Leaves'.'-'.$this->company_name); 
