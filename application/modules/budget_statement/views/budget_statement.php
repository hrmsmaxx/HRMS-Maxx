      ...
      <div class="content container-fluid">
		<div class="row">
			<div class="col-sm-12">
				<h4 class="page-title"><?=lang('budget_statement')?></h4>
			</div>
		</div>
    <?php $this->load->view('sub_menus');?>
		<div class="card-box budget-tabs m-b-0">
			<ul class="nav nav-tabs nav-tabs-solid nav-justified page-tabs">
        ...
        <div class="tab-content">           
           ...
           <?php  for ($i=$start_years; $i <=$current_year ; $i++) { ?>
            <div id="statement_<?php echo $i;?>" class="tab-pane <?php echo ($i == $current_year )?'active':'';?>">
              <div class="card-box">
                <h4 class="card-title"><?=lang('budget_statement')?> <?php echo $i;?></h4>
                ...
                <table class="table table-hover accounts-table budget-table m-b-0 table-nowrap">
                  ...
                  <tbody id="revenue_cont_<?php echo $i;?>">
                    <tr class="title-row">
                      <td colspan="10"><strong><?=lang('revenues')?></strong></td>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-xs add-new" id="revenue_add" data-year ="<?php echo $i;?>" onclick="revenue_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> <?=lang('add_revenue')?></button></td>
                    </tr>
                    <?php $this->db->select('*');
                                ...
                                <div class="modal-dialog">
                                  <div class="modal-content modal-md">
                                    <div class="modal-header">
                                      <h4 class="modal-title"><?=lang('delete_budget_revenue')?>( <?php echo $revenue['budget_name']?> )</h4>
                                    </div>
                  ...
                  <tbody>
                    <tr class="total-row">
                      <td><strong><?=lang('total_revenues')?></strong></td>
                  ...
                  <tbody id="expense_cont<?php echo $i;?>">
                    <tr class="title-row">
                      <td colspan="10"><strong><?=lang('expenses')?></strong></td>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-xs add-new" id="expense_add" data-year ="<?php echo $i;?>" onclick="expense_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> <?=lang('add_expenses')?></button></td>
                    </tr>
                    <?php $this->db->select('*');
                          $this->db->where("year",$i);
                            ...
                            foreach ($expenses as $expense) {?>
                              <tr>
                                ...
                                <div class="modal-dialog">
                                  <div class="modal-content modal-md">
                                    <div class="modal-header">
                                      <h4 class="modal-title"><?=lang('delete_budget_revenue')?>( <?php echo $expense['budget_name']?> )</h4>
                                    </div>
                  ...
                  <tbody>
                    <tr class="total-row">
                      <td><strong><?=lang('total_expenses')?></strong></td>
                      <?php $this->db->select('sum(jan) as jan,sum(feb) as feb,sum(mar) as mar,sum(apr) as apr,sum(may) as may,sum(jun) as jun,sum(jul) as jul,sum(aug) as aug,sum(sep) as sep,sum(oct) as oct,sum(nov) as nov,sum(dece) as dece');
                          $this->db->where("year",$i);
                          $this->db->where("type","expense");
                          $this->db->where("subdomain_id",$this->session->userdata('subdomain_id'));
                  ...
                  <tbody>
                    <tr>
                      <td><strong><?=lang('earnings_before_tax')?></strong></td>
                      <?php 
                        $profit = array();
                  ...
                  <tbody id="tax_cont<?php echo $i;?>">
                    <tr class="title-row">
                      <td colspan="10"><strong><?=lang('taxes')?></strong></td>
                      <td class="text-right" colspan="5"><button type="button" class="btn btn-info btn-xs add-new" id="" data-year ="<?php echo $i;?>" onclick="tax_add(<?php echo $i;?>)"><i class="fa fa-plus"></i> <?=lang('add_tax')?></button></td>
                    </tr>
                   <?php $this->db->select('*');
                          $this->db->where("year",$i);
                          ...
                          <?php   foreach ($taxes as $tax) {?>
                              <tr>
                                 ...
                                 <!--  <td data-name="feb" data-id="tax_feb<?php echo $tax_count.$i; ?>"></td>
                                  ...
                                  <td class="text-right actionss">
                                    <a class="btn btn-info btn-xs add"  onclick="save_tax(<?php echo $tax_count?>,<?php echo $i?>,this)" data-type ="tax" data-id ="<?php echo $tax['id'];?>" id="tax_save" title="<?=lang('Add')?>" data-toggle="tooltip"><i class="fa fa-check"></i></a>
                                    ...
                                    <a class="btn btn-danger btn-xs delete" data-toggle="modal" data-target="#tax_delete<?php echo $tax['id']?>" id="" title="<?=lang('delete')?>" data-toggle="tooltip"><i class="fa fa-trash-o"></i></a>
                                  </td>
                                <tr>
                                ...
                                <div class="modal-dialog">
                                  <div class="modal-content modal-md">
                                    <div class="modal-header">
                                      <h4 class="modal-title"><?=lang('delete_budget_revenue')?>( <?php echo $tax['budget_name']?> )</h4>
                                    </div>
                  ...
                  <tbody>
                    <tr class="total-row">
                      <td><strong><?=lang('total_taxes')?></strong></td>
                  ...
                  <tbody>
                    <tr class="total-row">
                      <td><strong><?=lang('net_earnings')?></strong></td>
                      <?php 
                        $net_earning = array();
