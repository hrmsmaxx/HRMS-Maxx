	...
	<div class="row">
		<div class="col-xs-8">
			...
			<ul class="breadcrumb m-b-30 p-l-0" style="background:none; border:none;">
						        ...
						        <li class="active"><a href="<?php echo base_url(); ?>jobs/apptitude_result"><?php echo lang('apptitude_results');?></a></li>
						        <li><?php echo lang('apptitude_answers');?></li>
			</ul>
		</div>
					 ...
					 foreach ($category_questions as $category_id => $questions){
					 	...
					 	foreach ($questions as $key => $quest_id) {
							...
							<span><b><?php echo lang('status')?> :</b>  <?php if($answer_status[$quest_id]==1) {
								echo '<label class="label label-success">'.lang('correct').'</label>';
							}else{
								echo '<label class="label label-danger">'.lang('wrong').'</label>';
							} ?></span>
							<?php
							...
							<span><b><?php echo lang('status')?> :</b>  
								...
								if($answer_status[$quest_id]==NULL){
									?>
									<a class="label label-success" href="<?php echo site_url('jobs/change_answer_status/').$aptitude_id[$quest_id].'/1/'.$this->uri->segment(3).'/'.$this->uri->segment(4);?>"><?php echo lang('correct');?></a>
									<a  class="label label-danger"  href="<?php echo site_url('jobs/change_answer_status/').$aptitude_id[$quest_id].'/0/'.$this->uri->segment(3).'/'.$this->uri->segment(4);?>"><?php echo lang('wrong');?></a>
									<?php 
								}else{

								 if($answer_status[$quest_id]==1) {
								?><label class="label label-success"><?php echo lang('correct');?></label><?php 
							}else{
								?><label class="label label-danger"><?php echo lang('wrong');?></label><?php 
							} ?></span>
					 				<?php 
					 			}
