...
class Jobs extends MX_Controller
{
    ...
    {   
      ...
      //print_r($_SESSION);exit;
        ...
        $this->template->title(lang('recruiting_process').' - '.$this->company_name);
      ...
      $total_jobs =  $this->Jobs_model->select_numrows('jobs',array('subdomain_id'=> $this->session->userdata('subdomain_id'))); /* total jobs count*/
      $data['total_jobs'] = $total_jobs->num_rows();
      $total_candidates =  $this->Jobs_model->select_numrows('registered_candidates',array('subdomain_id'=> $this->session->userdata('subdomain_id'))); /*job seekers count*/
     ...
     $cond['jobs.subdomain_id'] = $this->session->userdata('subdomain_id');
     ...
     $data['month_wise'] = $month_wise= $this->Jobs_model->select_month_wise_details(array('status'=>1,'jobs.subdomain_id' => $this->session->userdata('subdomain_id')));
	...
	public function manage()
    {
        ...
        $where['jobs.subdomain_id'] = $this->session->userdata('subdomain_id'); /* get only undeleted jobs*/
       ...
       // print_r($data['applications']);exit; 
        ...
        $data['job_types'] = $this->Jobs_model->select('jobtypes',array('status'=>1,'subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch job types*/
	...
	public function add()
    {
        ...
        $data['experience_levels'] = $this->Jobs_model->select('experience_level',array('subdomain_id'=>$this->session->userdata('subdomain_id')));
        $data['job_types'] = $this->Jobs_model->select('jobtypes',array('status'=>1,'subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch job types*/
        // echo "<pre>";print_r($data['job_types']);die;
        $data['departments'] = $this->Jobs_model->select('departments',array('subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch job types*/
    ...
    function candidates(){
        ...
        $data['candidates_list'] = $this->Jobs_model->select_candidates($where);
        //echo $this->db->last_query();exit; 
         $this->template
                ->set_layout('users')
    ...
    function candidates_board(){
      $this->load->module('layouts');
        ...
        $data['currencies'] = App::currencies();
        $data['languages'] = App::languages();
        $where1['registered_candidates.subdomain_id'] = $this->session->userdata('subdomain_id');
        
        $data['candidates_list'] = $this->Jobs_model->select_candidates($where1);
       ...
       $this->load->module('layouts');
        ...
        if($this->input->post('save_contact')){
          ...
          if($this->form_validation->run()){
            ...
            $basic_upd['subdomain_id'] = $this->session->userdata('subdomain_id');
    ...
    function add_experience(){
      if($this->uri->segment(3)!=""){
        ...
        $data['form_type'] = lang('edit');
      }else{
        $data['exp_level'] = [];
        $data['form_type'] = lang('Add');
      }
      if($this->input->post('submit')){

      $add['subdomain_id'] = $this->session->userdata('subdomain_id');
	...
	public function edit()
    {
      ...
      if($this->input->post('submit')){
        ...
        $data['experience_levels'] = $this->Jobs_model->select('experience_level',array('subdomain_id'=>$this->session->userdata('subdomain_id')));
        $data['job_types'] = $this->Jobs_model->select('jobtypes',array('status'=>1,'subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch job types*/
        $data['departments'] = $this->Jobs_model->select('departments',array('subdomain_id'=>$this->session->userdata('subdomain_id'))); /* to fetch job category*/
    ...
    public function apptitude_result(){
      ...
      $this->load->module('layouts');
        ...
        $where['jobs.subdomain_id'] = $this->session->userdata('subdomain_id');
        $data['completed_users'] = $this->Jobs_model->get_apps_completed_user($where);
        //echo $this->db->last_query();exit; 
        $this->template
                ->set_layout('users')
    ...
    public function question_answer()
    {
        ...
        $data['user_detail'] = $this->Jobs_model->select_row_array('registered_candidates',array('id'=>$user_id,'subdomain_id'=>$this->session->userdata('subdomain_id')));
        //echo $this->db->last_query();exit; 
 ...
 $data['time_list'] =$this->Jobs_model->time_diffence();
    ...
    public function job_portals()
    {
        ...
        $data['countries'] = $this->Jobs_model->select('countries');
        $data['experience_levels'] = $this->Jobs_model->select('experience_level',array('subdomain_id'=>$this->session->userdata('subdomain_id')));
        $data['departments'] = $this->Jobs_model->select('departments',array('subdomain_id'=>$this->session->userdata('subdomain_id')));
        $where['jobs.deleted_at'] = NULL; /* get only undeleted jobs*/
        $where['jobs.subdomain_id'] = $this->session->userdata('subdomain_id'); /* get only undeleted jobs*/
        $data['jobs_list'] = $this->Jobs_model->select_jobs_list($where);

        // echo "<pre>";print_r($data['jobs_list']);die;
