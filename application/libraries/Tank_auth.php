...
class Tank_auth
{
		...
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					if($this->ci->session->userdata('subdomain_id') !=0){
							$subscription_details = $this->ci->db->get_where('subscribers',array('subscribers_id'=>$this->ci->session->userdata('subdomain_id')))->row_array();
						
						}else{
							$subscription_details['status'] =1;
						}
					if($user->subdomain_id != $this->ci->session->userdata('subdomain_id')){
						$this->error = array('incorrect_domain' => 'Please check your Workspace');
					}elseif($subscription_details['status'] ==0){
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$role_name = $this->ci->users->getRoleName($user->user_type);
						$branch = $this->ci->users->getBranch($user->id);
	...
	*/
	...
	function admin_create_user($username, $email, $password, $first_name,$last_name,$dob, $gender, $id_code, $company, $city, $state, $address, $pincode, $role, $user_type, $phone, $doj, $email_activation, $designation_id, $department_id, $branch_id, $is_teamlead, $ro,$subdomain_id) 
	{
		...
		//$deptmnt_id = implode(',', $department_id);
		//$dept_id_code = $this->db->query("SELECT group_concat(id_code) as id_code FROM dgt_departments WHERE find_in_set(deptid, '$deptmnt_id'))")->result();
		//$deptidcode = $this->db->query("SELECT GROUP_CONCAT(`id_code`) as idcode FROM `dgt_departments` WHERE find_in_set(`deptid`, '$deptmnt_id')")->result_array();
		// echo 'id_code<pre>'; print_r($deptidcode); exit;
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');
	...
	//Get Branch id based on employees in users table
	function get_branch($id)
	{
		 return $this->ci->db->select('branch_id')->where('id',$id)->get('users')->row()->branch_id;
	}
}
