...
class Jobs_model extends CI_Model
{
   ...
   public function select($table,$where=NULL){
    ...
    function get_counts($table,$where){
     ...
     function get_counts_new($table,$where){
        $this->db->select('count(*) as count');
        $this->db->from($table);
        $this->db->join('jobs','jobs.id=candidate_job_status.job_id');
        if($where!=NULL){
     ...
     function get_latest_jobs($limit=NULL,$where =NULL){
     ...
     function select_candidate_details($where){
        ...
        $this->db->join('candidate_files as c','c.candidate_id=a.id');
        if($where!=NULL){
            $this->db->where($where);
        }
        $query = $this->db->get();
    ...
    function get_apps_completed_user($where =NULL){
    ...
    function select_candidates($where=NULL){
    ...
    {
        $this->db->select('count(dgt_candidate_job_status.id) as offered_count,user_job_status');
        $this->db->from('candidate_job_status');
        $this->db->join('jobs','jobs.id = candidate_job_status.job_id');
        if($where!=NULL){
        $this->db->where($where);    
    ...
    function select_month_wise_details($where=NULL){
        $this->db->select('MONTH(dgt_candidate_job_status.created_at) as month,job_id,candidate_id,user_job_status,dgt_candidate_job_status.id');
        $this->db->from('candidate_job_status');
        $this->db->join('jobs','jobs.id = candidate_job_status.job_id');
        if($where!=NULL){
        $this->db->where($where);    
