...
class Offer extends CI_Model
{
	...
	*/
	...
	static function job_where($array = NULL){
		 return self::$db->where('subdomain_id',$_SESSION['subdomain_id'])->get('jobtypes')->result();	
	}
	...
	static function approve_candidate($user_id = NULL){
		...
		$query = "SELECT ofs.*,ofa.status as approver_status,ofa.id as app_row_id from dgt_offers as ofs LEFT JOIN dgt_offer_approvers as ofa on ofs.id = ofa.offer  WHERE ofa.approvers=$user_id AND ofa.status in (1,2) AND view_status= 1";
