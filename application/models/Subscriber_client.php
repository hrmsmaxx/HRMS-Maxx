...
class subscriber_client extends CI_Model

{
	...
	*/
	...
	static function update($company, $data = array()) {
		...
		self::$db->where('subscribers_id',$company)->update('subscribers',$data);
...

	{
		...
		return self::$db->order_by('company_name','ASC')->get('subscribers')->result();
...

	{
		...
		$invoices = self::$db->where(array('client'=>$company,'status !='=>'Cancelled'))->get('subscriber_invoices')->result();

		foreach ($invoices as $key => $invoice) {

			if($invoice->currency != $cur){

					$due += Applib::convert_currency($cur,subscriber_invoice::get_invoice_due_amount($invoice->inv_id));

			}else{

				$due += subscriber_invoice::get_invoice_due_amount($invoice->inv_id);
...

	{
		...
		self::$db->join('subscribers','subscribers.subscribers_id = users.subdomain_id');	
		return self::$db->where('users.subdomain_id',$company)->get('account_details')->result();
	...
	static function payable($company){
		...
		$invoices = subscriber_invoice::get_client_invoices($company);

		foreach ($invoices as $key => $inv) {

			if($inv->currency != config_item('default_currency')){

				$total += Applib::convert_currency($inv->currency, subscriber_invoice::payable($inv->inv_id));

			}else{

				$total += subscriber_invoice::payable($inv->inv_id);
...

	{
		...
		return self::$db->where('subscribers_id',$company)->get('subscribers')->row();
...

	{
		...
		$client = self::$db->where('subscribers_id', $company)->get('subscribers')->result();
...

	{
		...
		$payments = self::$db->where(array('paid_by'=>$company,'refunded'=>'No'))->get('subscriber_payments')->result();
	...
	static function total_expenses($company = NULL)
		...
		static function month_amount($year, $month, $client){
	        ...
	        $query = "SELECT * FROM dgt_subscriber_payments WHERE paid_by = '$client' AND MONTH(payment_date) = '$month' AND refunded = 'No' AND YEAR(payment_date) = '$year'";
	...
	$company_invoices 	= subscriber_invoice::get_client_invoices($company);
	...
	$company_contacts 	= self::get_client_contacts($company);
			...
			if (count($company_invoices)) {
				...
				foreach ($company_invoices as $invoice) {
					...
					self::$db->where('invoice_id',$invoice->inv_id)->delete('subscriber_items');
			...
			self::$db->where('client',$company)->delete('subscriber_invoices');
			...
			self::$db->where('paid_by',$company)->delete('subscriber_payments');
			...
			self::$db->where('subscribers_id',$company)->delete('subscribers');
