...
class User extends CI_Model
{
    ...
    static function team() {
        return self::$db->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'role_id !='=>2,'role_id !='=>1,'activated'=>1,'banned'=>0)) ->get('users')->result();
    }

    static function teamlead() {
    	$branch_id = $_SESSION['branch_id'];
        if($branch_id != '') {
            self::$db->where("branch_id IN (".$branch_id.")",NULL, false);
        }
        return self::$db->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'role_id !='=>2,'role_id !='=>1,'activated'=>1,'banned'=>0,'is_teamlead'=>'yes')) -> get('users')->result();
    }
    ...
    static function employee() {
        ...
        return self::$db->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'role_id'=>3,'activated'=>1,'banned'=>0)) -> get('users')->result();
    }
    ...
    static function all_staffs_users(){
        ...
        return self::$db->where('subdomain_id',$_SESSION['subdomain_id'])->where('role_id',3)->get('users')->result();
    }
    ...
    static function admin_list(){
        return self::$db->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'role_id !=' => 2,'role_id !=' => 3,'activated' => 1))->get('users')->result();
    }

    // Get all user list
    static function user_list(){
        return self::$db->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'role_id' => 2,'activated' => 1))->get('users')->result();
    }

    // Get staff list
    static function staff_list(){
        return self::$db->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'role_id' => 3,'activated' => 1))->get('users')->result();
    }

    // Get roles
    static function get_roles(){
        return self::$db->where('subdomain_id',$_SESSION['subdomain_id'])->get('roles')->result();
    }
    ...
    static function all_projects(){
        return self::$db->where('subdomain_id',$_SESSION['subdomain_id'])->get('projects')->result();
    }
    ...
    {
        $where = array('US.subdomain_id'=>$_SESSION['subdomain_id'],'US.activated'=>1,'US.role_id'=>3);
    ...
    {
        return self::$db->get_where('departments',array('deptid'=>$dept_id))->row()->deptname;
    }
    public function GetDesignationNameById($designation_id)
    {
        return self::$db->get_where('designation',array('id'=>$designation_id))->row()->designation;
    }
    public function GetJobType()
    {
        return self::$db->select('id,job_type,status')->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'status'=>1))->get('jobtypes')->result_array();
    }
    public function get_vocations()
    {
        return self::$db->select('*')->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'status'=>1))->get('vocations')->result_array();
    }

    public function get_annual_incentive_plans()
    {
        return self::$db->select('*')->where(array('subdomain_id'=>$_SESSION['subdomain_id'],'status'=>1))->get('annual_incentive_plans')->result_array();
    }
