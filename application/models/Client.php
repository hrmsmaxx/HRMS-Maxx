...

{
	...
	*/
	...
	{
		//Get Admin branch based on companies - Admin Login by Thamayanthi.V
		/*if($_SESSION['role_name']=='admin' && $_SESSION['branch_id'] != '') {
			$subdomain_id = $_SESSION['subdomain_id'];
			$branch_id = $_SESSION['branch_id'];

			$records = self::$db->query("SELECT * FROM `dgt_companies` WHERE find_in_set(`branch_id`, '$branch_id') AND `is_lead`='0' AND `co_id`>='0' AND `subdomain_id`='".$subdomain_id."' ")->result();
			return $records;
		} else {*/
			//Get all companies - Super Admin login
