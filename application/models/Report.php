...
class Report extends CI_Model
{
	...
	static function month_amount($year,$month){
        $total = 0;
        $query = "SELECT * FROM dgt_payments WHERE MONTH(payment_date) = '$month' AND refunded = 'No' AND YEAR(payment_date) = '$year'";
    ...
    static function year_amount($year){
        $total = 0;
        $query = "SELECT * FROM dgt_payments WHERE refunded = 'No' AND YEAR(payment_date) = '$year'";
        $payments = self::$db->query($query)->result();
        foreach($payments as $p) {
            $amount = $p->amount;
            if ($p->currency != config_item('default_currency')) {
    ...
    static function total_paid(){
        $total = 0;
        $query = "SELECT * FROM dgt_payments WHERE refunded = 'No'";
        $payments = self::$db->query($query)->result();
        foreach($payments as $p) {
            $amount = $p->amount;
            if ($p->currency != config_item('default_currency')) {
