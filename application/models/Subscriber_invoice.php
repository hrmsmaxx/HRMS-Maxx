...
class Subscriber_invoice extends CI_Model
{
    private static $db;
    ...
    {
        return self::$db->where('inv_id', $invoice)->get('subscriber_invoices')->row();
    }
    ...
    public static function get_invoices($limit = null, $status = null, $cancelled = false)
    {
        ...
        if($cancelled) :
            return self::$db->where(array('inv_id >' => 0, 'status !=' => 'Cancelled'))->get('subscriber_invoices')->result();
        else:
        return self::$db->order_by('inv_id', 'desc')->where(array('inv_id >' => 0, 'inv_deleted' => 'No'))->get('subscriber_invoices', $limit)->result();
        endif;
    }
    ...
    {
        return self::$db->where('inv_id', $invoice)->update('subscriber_invoices', $data);
    }

    // Save Invoice
    public static function save($data)
    {
        self::$db->insert('subscriber_invoices', $data);
    ...
    {
        return self::$db->insert('subscriber_items', $data);
    }

    // Save tax rates
    public static function save_tax($data)
    {
        return self::$db->insert('subscribers_tax_rates', $data);
    }

    // Get tax rate using ID
    public static function tax_by_id($id)
    {
        return self::$db->where('tax_rate_id', $id)->get('subscribers_tax_rates')->row();
    }
    // Update tax rate
    public static function update_tax($id, $data)
    {
        return self::$db->where('tax_rate_id', $id)->update('subscribers_tax_rates', $data);
    }

    // Delete tax rate from DB
    public static function delete_tax($id)
    {
        return self::$db->where('tax_rate_id', $id)->delete('subscribers_tax_rates');
    }

    public static function view_item($id)
    {
        return self::$db->where('item_id', $id)->get('subscriber_items')->row();
    }
    ...
    {
        return self::$db->select_sum('total_cost')->where('invoice_id', $invoice)->get('subscriber_items')->row()->total_cost;
    }

    public static function get_invoice_paid($invoice)
    {
        return self::$db->select_sum('amount')->where(array('invoice' => $invoice, 'refunded' => 'No'))->get('subscriber_payments')->row()->amount;
    }
    ...
    public static function overdue()
    {
        ...
        $sql = "SELECT * FROM dgt_subscriber_invoices WHERE DATE(due_date) <= '$date_today' AND status = 'Unpaid'";
    ...
    {
        return self::$db->get('subscribers_tax_rates')->result();
    }
    ...
    public static function has_items($id, $type = 'invoice')
    {
        ...
        return self::$db->where($type.'_id', $id)->order_by('item_order', 'asc')->get('subscriber_items')->result();
    }
    ...
    {
        return self::$db->where(array('module_field_id' => $invoice, 'module' => 'subscriber_invoices'))
    ...
    {
        return self::$db->where(array('client' => $company,'status !=' => 'Cancelled','show_client' => 'Yes'))->get('subscriber_invoices')->result();
    }
    ...
    public static function paid_invoices($company = null)
    {
        ...
        return self::$db->where(array('status' => 'Paid', 'inv_id >' => 0))->get('subscriber_invoices')->result();
    }
    ...
    public static function generate_invoice_number()
    {
        ...
        $query = self::$db->query('SELECT reference_no, inv_id FROM '.$dbPrefix.'subscriber_invoices WHERE inv_id = (SELECT MAX(inv_id) FROM '.$dbPrefix.'subscriber_invoices)');
    ...
    public static function ref_exists($next_number)
    {
        ...
        $records = self::$db->where('reference_no', config_item('invoice_prefix').$next_number)
            ->get('subscriber_invoices')->num_rows();
        if ($records > 0) {
    ...
    public static function by_range($start, $end, $report_by = null)
    {
        ...
        $sql = "SELECT * FROM dgt_subscriber_invoices WHERE $report_by BETWEEN '$start' AND '$end'";
    ...
    public static function recurring_invoices($company = null)
    {
        ...
        return self::$db->where(array('recurring' => 'Yes', 'inv_id >' => 0))->get('subscriber_invoices')->result();
    }

    public static function evaluate_invoice($id)
    {
        $has_balance = self::get_invoice_due_amount($id);
        $has_items = self::$db->where('invoice_id', $id)->get('subscriber_items')->num_rows();
        $is_cancelled = (self::view_by_id($id)->status == 'Cancelled') ? true : false;
        if(!$is_cancelled) :
            if ($has_items == 0 || $has_balance > 0) {
                self::$db->set('status', 'Unpaid')->where('inv_id', $id)->update('subscriber_invoices');
            } else {
                self::$db->set('status', 'Paid')->where('inv_id', $id)->update('subscriber_invoices');
            }
        endif;
    ...
    public static function delete($invoice)
    {
        ...
        self::$db->where(array('invoice_id' => $invoice))->delete('subscriber_items');
        //delete invoice payments
        self::$db->where(array('invoice' => $invoice))->delete('subscriber_payments');
        //clear invoice activities
        self::$db->where(array('module' => 'subscriber_invoices', 'module_field_id' => $invoice))->delete('activities');
        //delete invoice
        self::$db->where(array('inv_id' => $invoice))->delete('subscriber_invoices');
    }
    ...
    {
        return self::$db->where('inv_id', $invoice_id)->get('subscriber_invoices')->row_array();
    }
