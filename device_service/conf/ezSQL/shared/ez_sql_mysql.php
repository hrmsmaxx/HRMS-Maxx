...
class ezSQL_mysql extends ezSQLcore
{
	...
	function query($query, $indexed = false)
	{
		...
		// Query was a select
			...
			while ($row = @mysql_fetch_object($this->result)) {
				...
				if (!$indexed) {
					...
					// Search for a id column, else num_rows
					if (isset($row->id)) $this->last_result[$row->id] = $row;
					else $this->last_result[$num_rows] = $row;
				}
				$num_rows++;
			}
