...
class ezSQLcore
{
	...
	function query_treatment($query, $tables = array())
	{
		global $company;
		if ($tables === null) {
			return $query;
		...
		if ($company) {
			...
			if ($query) {
				if ($command === "INSERT") {
					$firstPosition = strpos($query, "(");
					echo $firstPosition;
					$query = substr_replace($query, "subdomain_id,", $firstPosition + 1, 0);
					$secondPosition = strpos($query, "(", $firstPosition + 1);
					$query = substr_replace($query, "$company->subdomain_id,", $secondPosition + 1, 0);
				} else if ($command === "UPDATE" || $command === "DELETE") {
					if (!empty($tables)) {
						$condition = "";
						foreach ($tables as $table) {
							$condition .=  "$table.subdomain_id = $company->subdomain_id AND ";
						}
						$query = preg_replace('/(WHERE(?!.*WHERE))/',  "WHERE $condition", $query);
					} else {
						$query = preg_replace('/(WHERE(?!.*WHERE))/',  "WHERE subdomain_id = $company->subdomain_id AND", $query);
					}
	...
	function put($query = null, $tables = "", $output = OBJECT, $y = 0)
	{
		$query = $this->query_treatment($query, $tables);
		// Log how the function was called
	...
	function get_row($query = null, $tables = "", $output = OBJECT, $y = 0)
	{
		$query = $this->query_treatment($query, $tables);
	...
	function get_col($query = null, $x = 0)
	{
		...
		$query = $this->query_treatment($query);
		$new_array = array();
	...
	function get_results($query = null, $tables = "", $outputAux = OBJECT, $indexedAux = false)
	{

		$query = $this->query_treatment($query, $tables);
		if (is_bool($outputAux)) {
			$output = OBJECT;
			$indexed = $outputAux;
		} else {
			$output = $outputAux;
		...
		// If there is a query then perform it if not then use cached results..
		if ($query) {
			if (!$indexed) $this->query($query);
			else $this->query($query, true);
		}
	 ...
	 *
	 *     $db->put("INSERT INTO users SET ".$db->get_set($db_data));
	 *
	 *     ...OR...
	 *
	 *     $db->put("UPDATE users SET ".$db->get_set($db_data)." WHERE user_id = 1");
	 *
	 * Output:
	 *
