...
function get_call_notification_job(){
  $.get(base_url+'candidate_chats/get_call_notification',function(res){
    // console.log(res); return false;
    var obj = jQuery.parseJSON(res);
    var status = obj.status;
    var obj = obj.data;
...
var notify_new = setInterval(get_call_notification_job, 5000);


/*initialize session for job interview video call */

function initializeSessionJob() {

  clearInterval(notify_new);

  var call_type = $('#new_call_type').val(); // one to one 0r group 
  var type_of_call = $('#call_type').val(); // audio or video 
  var to_id = $('#to_id').val(); 

//alert(to_id,type_of_call,call_type);
   
  // return false;

  $.post(base_url+'candidate_chats/get_chat_token',{call_type:call_type,type_of_call:type_of_call,to_id:to_id},function(res){
    ...
    var publisher = OT.initPublisher('member_tab', publisherOptions, handleError);
    $('#group_outgoing_caller_image').addClass('hidden');
    $('.user_vcend,.vccam').removeClass('hidden');  
    $('.menu').addClass('disabled');      
    // Connect to the session
    session.connect(token, function callback(error) {
      if (error) {
