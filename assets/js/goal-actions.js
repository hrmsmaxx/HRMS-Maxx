;(function($){
	$(document).ready(function(){
		...
		var addTask = function(){
			...
			var newTask = $('#new-goal').val();
			// If new task is blank show error message
			if(newTask == ''){
				$('#new-goal').addClass('error');
		...
		var closeNewTaskPanel = function(){
			$('#add_goal_action').toggleClass('visible');
			$('.new-task-wrapper').toggleClass('visible');
			if($('#new-goal').hasClass('error')){
				$('#new-goal').removeClass('error');
		...
		$('#add_goal_action').click(function(){
		...
		$('#new-goal').keydown(function(event){
