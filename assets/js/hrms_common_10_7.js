        ...
        if (recordscount > 0) {
            for (var i = 0; i < recordscount; i++) {
                ...
                if(record.avatar != 'default_avatar.jpg'){
                    ...
                    imgs = '<span class="avatar">' + fullname.charAt(0).toUpperCase() + '</span>';
                }
...
//      $('#new_hidden_rows').val(count);
  ...
  // $(document).on("click", "#revenue_save", function(){
  //   var empty = false;
...
//        $('#new_hidden_rows').val(count);
  ...
  // Add row on add button click
  // $(document).on("click", "#tax_save", function(){
  //   var empty = false;
  //   var input = $(this).parents("tr").find('input[name="text"]');
  //       input.each(function(){
  //     if(!$(this).val()){
  //       $(this).addClass("error");
  //       empty = true;
